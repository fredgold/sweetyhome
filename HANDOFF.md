...
`style.css` 단독 수정(+17/-4줄). 기존 DOM과 대시보드 렌더·이벤트
경로를 그대로 활용할 수 있어 `nav.js`·`index.html`은 무접촉,
스키마와 `BACKLOG.md`도 변경하지 않았다.

- 대시보드의 문서형 max-width clamp를 해제해 전체폭 프레임으로
  전환했다.
- 1440px 이상에서 대시 패널을 `4fr:3fr` 2열 그리드로 배치:
  여정 타임라인은 상단 전폭, 자산·매물 요약 카드 2장은 좌측 같은
  행, Top3는 우측 같은 행, 수집함 안내 배너는 하단 전폭이다.
- 1439px 이하는 기존 block 흐름
  (여정→요약 카드→Top3→안내)을 그대로 유지한다. 390px에서는 기존
  요약 카드 1열도 유지된다.

**검증**: Playwright 실제 상태 fixture. 1440/1920/2560px에서
좌측 요약/우측 Top3 폭은 각각 792/594, 1066/800, 1432/1074px,
wrap 폭은 viewport와 동일했다. 1440×900에서 콘텐츠 하단 624px,
패널 하단 867px로 첫 화면에 243px 여유가 있고 세 규격 모두 패널
추가 스크롤 0, document `maxScroll=0`, 가로 넘침 0. 1439px는
기존 세로 흐름, 390px는 1열+패널 내부 스크롤(document 0)로 복귀.
마일스톤·자산/매물 수치, Top3 추가/완료/별표 저장 왕복,
Top3 더보기와 자산·매물·수집함 `data-goto` 전환을 확인했고 대상
패널은 모두 `scrollTop=0`으로 열렸다. 안내 배너·액션/마일스톤 XSS
escape, `node --check js/nav.js`, `git diff --check` 통과.

스크린샷은 로컬·미커밋:
`mockups/b104-5-dash-compact-{1440,1920,2560}.png`,
`mockups/b104-5-dash-compact-mobile-390.png`.

- **B-104-5 완료·push 완료. B-104 에픽 코드 소진.**
- 에픽 종결 판정은 사용자 대형 모니터 전 탭 확인 후.
- **다음 손 B 작업: B-114(별도 지시).**

---

# 이전 핸드오프 — B-116 완료 (2026-07-19)
...
`index.html`+`style.css`만 제품 코드 수정. 기존 자산 렌더 ID와
이벤트 경로를 그대로 재사용할 수 있어 `assets.js`·`nav.js`는
무접촉이며 스키마도 변경하지 않았다. `BACKLOG.md`도 무접촉.

- 1440px 이상에서 자산 탭의 문서형 max-width clamp를 해제하고
  전체폭 프레임을 좌측 요약·목표·Tiptap 노트와 우측 등록부 표
  2구역으로 분리했다. 좌:우는 `3fr:8fr`이며 좌측 요약 카드 2개는
  세로 1열, 등록부는 우측 한 구역 전체를 사용한다.
- 1439px 이하는 새 래퍼가 block으로 돌아가 기존
  요약→등록부→목표·노트 순서를 그대로 유지한다. 390px의 등록부
  2열 카드(B-73), 필드 라벨·44px 입력 높이도 무변경이다.
- **좌측 sticky는 채택하지 않음.** 1440×900 실데이터 24건에서
  좌측 콘텐츠 높이 약 785px가 패널 가시 높이 775px보다 길었다.
  Tiptap 자체 스크롤과 함께 좌측을 고정하면 하단 저장·안내 영역
  도달성과 포커스 자동 스크롤이 나빠질 수 있어 앱쉘의 자연스러운
  단일 패널 스크롤을 유지했다. 패널 `scrollTop=360`에서 좌측도
  정확히 360px 이동하고, 에디터 클릭 시 포커스 항목이 자동으로
  가시 영역에 들어오는 것을 확인했다.

**검증**: Playwright 24건 fixture. 1440/1920/2560px에서 실제
2구역과 전체폭(좌/우 폭 380/964, 497/1327, 672/1792px)을 확인했고,
1439/390px는 기존 1열 순서로 복귀했다. 전 규격 document
`maxScroll=0`, 패널 `overflow-y:auto`+overscroll contain, 가로
넘침 0. 실제 Tiptap으로 포커스·편집·저장 API 왕복과 중첩 목록
Backspace 승격→Undo 복원·텍스트 보존을 확인했다. 등록부 인라인
편집·행 추가/삭제·붙여넣기 임포트 모달, B-106 상태 용량 표시도
통과. `node --check js/assets.js js/nav.js`, `git diff --check`
통과.
...
- **B-104-4 완료·push 완료**.
- **다음 B-104 작업: B-104-5 대시 D2(별도 지시).**

---

# 이전 핸드오프 — B-113 완료 (2026-07-19)

## 최신 작업: 로그인 세션 30일 슬라이딩 연장

```
8d7e504 fix: 로그인 세션 30일 슬라이딩 연장 (B-113)
```

`js/auth.js`+`api/_auth.js` 단독 수정(+15/-5줄). `state.js`/
`properties.js`/`index.html`/`BACKLOG.md` 전부 무접촉. 손 B의
B-104-4(`assets.js`+`style.css`+`nav.js`)와 파일 충돌 없음.

B-65의 24h TTL이 실사용에 짧아 모바일 재로그인이 잦다는 사용자
피드백 반영 — 개인용 PIN 앱이라 30일로 늘리고, 활성 사용 중엔
계속 연장되는 슬라이딩 방식으로 바꿨다.

- **서버(`api/_auth.js`)**: `SESSION_TTL` 86400초(24h)→
  `30*24*60*60`초(30일). `verifySession()`이 세션 존재를 확인한
  직후 `redis.expire(key, SESSION_TTL)`로 즉시 TTL을 재설정 —
  API를 계속 쓰는 한 Redis 세션이 만료되지 않는다. 만료/미인증
  분기(401 응답, 에러 메시지)는 그대로 유지.
- **클라(`js/auth.js`)**: `SH_TOKEN_TTL_MS` 24h→30일로 서버와
  수동 동기화(B-65 규칙 주석 갱신). `getToken()`이 토큰을 유효한
  것으로 반환하는 시점마다 `sh_token_exp`도 같이 30일 뒤로
  미룸 — 서버가 슬라이딩으로 계속 살아있는데 클라 로컬 시계가
  먼저 만료 판정해 불필요한 재로그인을 강제하는 불일치를 방지.
  `authHeaders()`는 내부적으로 `getToken()`을 거치므로 API 호출
  때마다 자연히 슬라이딩된다.
- B-80의 401→`setSyncState('expired')`+`forceLogin()` 경로(실제
  서버 거부 시 재로그인 유도)는 무변경 — 클라 슬라이딩은 로컬
  선제 판정 기준만 서버와 맞추는 것이고, 실제 인증 실패 처리는
  여전히 서버 401 응답이 유일한 트리거.

**검증**: 실 Redis 없이 코드 로직만으로 검증. ①서버 —
`api/_auth.js`를 실제 import해 Upstash SDK가 보내는 fetch 요청을
가로채는 방식으로 `verifySession()` 13개 체크(유효 토큰→get+
슬라이딩 expire 정확한 키·TTL로 호출·401 미설정, 만료/미존재
세션→401+에러메시지+expire 미호출, 인증 헤더 없음→401+redis
미접촉) 전부 통과. ②클라 — `auth.js`의 DOM 비의존 함수만 추출해
Node vm 샌드박스에서 `Date.now()`를 모킹해 11개 체크(29일 경과 후
유효+exp 재연장, 31일 무사용 경과 후 만료+clearToken, 60일간 매일
사용 시뮬레이션에서 계속 유효, `authHeaders()` 경유 슬라이딩,
게스트 모드 무토큰 무크래시) 전부 통과. `node --check` 2파일
통과.

**⚠️ 서버 파일 변경 — 배포 후 사용자 실로그인 확인 필요.** 로컬
검증은 Upstash 프로토콜을 모킹한 것이라 실제 Redis TTL 갱신·실제
PIN 로그인 왕복은 배포 환경에서 사용자가 직접 확인해야 한다.
...
# 이전 핸드오프 — B-104-3 수집함 카드/피드 뷰 재정립 완료 (2026-07-19)

## 최신 작업: 카드뷰 기본 3열+ · 리스트뷰 중앙 1열 피드

`js/scraps-render.js`+`style.css`만 제품 코드 수정. 수집함을
“훑어보는 카드뷰”와 “읽고 관리하는 리스트뷰”로 명확히 분리하고,
수집함 탭만 문서형 max-width clamp를 해제했다. `BACKLOG.md`·
스키마·`index.html`·`nav.js`·`scraps-form.js`는 무접촉.

- 기본 진입은 gallery. 뷰 상태는 기존처럼 메모리에만 두며,
  `renderScraps()`나 탭 왕복 시 사용자의 현재 선택을 강제 초기화하지
  않는다.
- 카드뷰는 제목·유형·상태·대표 사진·핵심 메타 최대 2개만 표시.
  원문·상태 셀렉트·버튼은 넣지 않고 카드 클릭/Enter로 기존 편집
  모달을 연다. 사진이 없으면 8개 유형별 색 프리뷰를 표시한다.
- 1440/1920/2560px는 auto-fill 3/4/6열, 1200~1439px는 정확히
  2열, 768/390px는 1열. 수집함 wrap은 900px 이상 전체폭이며
  그리드가 폭을 흡수한다.
- 리스트뷰는 전체폭 프레임 안의 중앙 `max-width:880px` 1열 피드.
  현행 상세 카드의 renderMd·B-97 원문 토글·더보기·상태 셀렉트·
  수정/자격확인/삭제·사진·태그를 전부 유지한다.
- **B-95 수집함 2열은 은퇴**: 상세 원문·관리 버튼을 담은 카드를
  2열로 병치하면 “훑기”와 “읽기”가 섞여 두 기능 모두 길고
  산만했다. 밀도 역할은 새 카드 그리드로 옮기고, 상세 리스트는
  읽기 폭 1열로 되돌렸다. 액션 B-95/AC3 규칙은 영향 없음.
- 유형/상태의 비정상 값은 CSS 클래스에 직접 넣지 않도록 안전한
  fallback 클래스를 쓰고 표시 텍스트는 `esc()` 처리. 기존
  `renderMd()`·태그 `esc()` 경로와 데이터 스키마는 그대로다.

**검증**: Playwright 동일 36건. 1440px 기존 B-95 상세 2열 높이
7,745px → 새 카드 3열 3,717px(`-4,028px`, **-52.0%**).
1440/1920/2560/1200/1439/768/390에서 열 수 3/4/6/2/2/1/1,
전체 desktop wrap 폭=viewport, 전 규격 document `maxScroll=0`,
패널 `overflow-y:auto`+overscroll contain, 가로 넘침 0. 두 뷰
왕복+탭 왕복 상태 보존, 카드 클릭/키보드→편집 모달, 검색·유형
필터·정렬·상태 변경 모킹 저장·B-97 원문↔서식 토글, 제목·태그 XSS
페이로드 전부 통과. `node --check js/scraps-render.js`,
`git diff --check` 통과.
...
- **B-104-3 완료**. B-112의 `style.css` 선행 변경
  (`5461c78`) push 뒤 최신화해 충돌 없이 적용.
- **다음 B-104 작업: B-104-4 자산 A2(별도 지시).**

---

# 이전 핸드오프 — B-112 완료 (2026-07-19)

## 최신 작업: 매물 추가 폼 열림 시 rail 하단 필터 칩 숨김

```
5461c78 fix: 매물 추가 폼 열림 시 rail 하단 필터 칩 숨김 (B-112)
```

`style.css` 단독 수정(+5줄). `properties.js`/`index.html`/
`state.js`/`BACKLOG.md` 전부 무접촉.

사용자 실기기 스크린샷 확보 버그 — 매물탭 추가 폼(`#form`) 열림
상태로 rail을 최하단까지 스크롤하면 필터 칩 바(`#cxFilterBar`)가
폼 아래에 노출됨. `#form`과 `#cxFilterBar`가 같은 `<section>`의
형제 요소이고 데스크톱 기본이 `#cxFilterBar`를 일반 흐름 가로
바로 두는 구조라, 폼이 열려 rail이 길어지면서 필터 바까지
스크롤 영역에 들어와 생긴 문제였다.

- 기존 `.form.open` 상태 셀렉터를 그대로 활용해 순수 CSS 형제
  셀렉터 `.form.open~#cxFilterBar{display:none;}` 한 줄로 해결 —
  JS 배선 불필요, DOM 제거가 아니라 `display` 토글이라 필터 칩의
  `.on` 선택 상태는 그대로 보존.
- 이 규칙이 모바일 지도뷰 고정 오버레이 규칙보다 특이도가 높아
  모바일 하단 시트가 열릴 때도 필터 바가 같이 숨는데, 시트가
  이미 필터 바를 시각적으로 덮던 것과 결과가 같아 회귀 아님.
- 리스트뷰는 `syncListToolbar`가 `#cxFilterBar`를
  `#cxListToolbar`로 실제 이동시켜 `#form`의 형제가 아니게 되므로
  이 규칙 미적용 — 상단 sticky 툴바라 애초 이 버그 대상 밖.
- `renderComplexes()`의 단지 0개 인라인 강제 숨김(2316행)과는
  인라인이 항상 이겨 무충돌.

**검증**: Playwright(로컬 node UTF-8 정적 서버, guest 로그인+
테스트 단지 1건 주입) 데스크톱 1440×900+모바일 390×844 13개 체크
전부 통과 — 폼 열기 전/후 필터 바 노출 전환, rail 최하단 스크롤
후에도 계속 숨김, 취소/토글 두 닫기 경로 각각 필터 바 복원+기존
선택값 유지, 리스트뷰 전환 무충돌, 모바일 시트 열림/닫힘 동일
동작. `node --check` 통과.

- **B-112 완료·push 완료**. 손 B의 B-104-3
  (`scraps-render.js`+`style.css`) 미커밋 변경 없음을 `git status`
  로 확인 후 진행, 셀렉터 충돌 없음.
- **다음**: B-113(별도 지시).

---

# 이전 핸드오프 — B-111 완료(커밋①②)·에픽 실기기 이슈 전부 해소 (2026-07-19)
