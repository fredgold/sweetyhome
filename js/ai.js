...
function stateSnapshot(){
  ...
  const props=state.complexes.map(cx=>{
    const rep=cxRepOf(cx);
    return `- ${cx.complexName||'무명'} | ${cx.complexStatus||'관심'} | 보증금 ${rep&&rep.deposit!=null?rep.deposit:'?'}억 | 전용 ${rep&&rep.areaM2!=null?rep.areaM2:'?'}㎡ | ${cx.loc||''}${cx.aiScore!=null?` | AI ${cx.aiScore}점`:''}`;
  }).join('\n')||'(없음)';
