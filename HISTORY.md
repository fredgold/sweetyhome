...
## 2026-07-19 — B-53: 헤더+탭바 단일 topbar 고정

- `header`와 `.apptabs`를 `#appTopbar` 하나로 감싸 고정 정책을
  `position:sticky; top:0`으로 통일했다. 데스크톱(≥900px)은
  브랜드·탭·헤더 액션이 같은 줄, 모바일은 같은 wrapper 안에서
  헤더와 가로 스크롤 탭이 두 줄로 유지된다. 스크롤 시 어느 한쪽만
  사라지던 기존 이원화는 제거했다.
- `nav.js`가 wrapper 실측 높이를 단일 `--app-top-h`로 기록하고
  `ResizeObserver`로 폰트·반응형 재배치까지 동기화한다. 모바일 매물
  패널, 지도 오버레이, 목록 sticky toolbar와 데스크톱 높이 fallback이
  모두 이 변수만 소비한다. `properties.js`가 기록하는 레거시
  `--topbar-h`·`--nav-h`·`--overlay-top`은 파일 락 때문에 그대로
  두었으나 위 배치 계산에서는 더 이상 사용하지 않는다.
- B-98의 `--props-panel-top`은 기존처럼 활성 매물 패널의
  `getBoundingClientRect().top + scrollY`를 RAF로 실측한다. wrapper
  도입 뒤 데스크톱 패널 top `92.5px`, bottom `900px`, 1440×900과
  1920×900 모두 `document maxScroll=0`으로 회귀 기준을 유지했다.
- Playwright: 1440×900·1920×900에서 5개 탭 전환/가로 overflow 0,
  액션탭 `scrollY=285`에서도 topbar `top=0`; 390×844에서 topbar
  높이 `158.5px`가 스크롤 전후 동일, 매물 지도 `maxScroll=0`,
  목록 toolbar sticky top `158.5px`, 지도↔목록·추가 폼 시트 정상.
  모달·폼 시트 `z-index=1100` > topbar `1001`도 확인했다. 변경
  전후 및 스크롤 스크린샷은 `/tmp/b53-before`, `/tmp/b53-after`에서
  육안 비교했다. `node --check js/nav.js`, `git diff --check` 통과.
- 제품 코드 변경은 `index.html` +2줄, `nav.js` +5줄,
  `style.css` +22/-21줄(총 50줄)로 지시 범위(30~70줄) 안이다.
  병행 중인 손 A의 `js/utils.js`·`js/assets.js`·`js/scraps-form.js`·
  `js/scraps-render.js`와 `properties.js`, `BACKLOG.md`는 무접촉.

---

## 2026-07-19 — B-103 2단계 ①②: Tiptap 프로덕션 전환 — 자산 노트·수집함 폼/모달 (커밋 2개)

채택 확정(Tiptap 17/17)에 따른 실제 프로덕션 반영. **`master`에 코드로
반영되는 첫 커밋**(1·2차 PoC는 `poc` 브랜치에만 존재, 여기서부터
master 이력 시작). 지시 조건대로 커밋②(수집함)까지만 진행하고
보고 — 커밋③(매물 메모 멀티 인스턴스)은 검증 후 별도 지시 대기.

**파일 락 준수**: `index.html`·`nav.js`·`style.css` 전부 무접촉(손 B
B-53과 동시 진행 확인, `git status`로 매 커밋 전 확인). 에디터
마운트·스타일은 전부 JS로만 처리 — 새 DOM은 만들되(자산 노트 1곳만,
`insertAdjacentElement`) index.html 자체는 안 건드림, 스타일은 기존
`.sc-md-editor`/`.sc-md-content` 클래스 재사용(신규 클래스 0개, 이
클래스들이 h1~h3/ul/ol/blockquote CSS를 이미 갖고 있어 별도 스타일
불요 — PoC 때 이미 확인된 특성). 새 JS 파일 없음 — 에디터 래퍼는
전부 `utils.js`에.

### 공용 래퍼(`utils.js`)

- `loadTiptapMods()` — `@tiptap/core`+`@tiptap/starter-kit`+
  `tiptap-markdown`+`@tiptap/suggestion`(신규, 슬래시 커맨드용) 4개를
  esm.sh에서 동시 로드하는 캐시 프라미스. 실패해도 예외를 밖으로
  던지지 않고 호출부가 `.catch(()=>null)`로 받아 폴백 판단.
- `buildTiptapSlashExtension(mods,menuElId,items,onSelect)` —
  `@tiptap/suggestion` 기반 슬래시 메뉴. 기존 `.slash-menu`/
  `.slash-item` CSS·DOM(`#sc_slashMenu`/`#sem_slashMenu`, 이미
  index.html에 있던 빈 div)을 그대로 재사용해 새 팝업 라이브러리
  (tippy.js 등) 없이 기존과 동일한 위치·모양을 재현.
  **실측으로 발견한 API 함정**: `@tiptap/suggestion`의 `onStart`/
  `onUpdate` 콜백 `props`에는 `.command` 함수가 있지만, `onKeyDown`의
  `props`는 `{view,event,range}`뿐이라 `.command`가 없음(격리
  테스트로 확인) — Enter 키 처리 시 `props.command is not a function`
  으로 깨짐. `onStart`/`onUpdate`에서 받은 `command`를 클로저에
  저장해 `onKeyDown`(Enter)에서 재사용하는 방식으로 해결.
  **두 번째 함정**: 방향키(↑↓)로 항목 이동 시 메뉴를 다시 그리는
  `renderMenu()`가 `innerHTML`을 통째로 재조립해 이전 클릭 핸들러를
  날려버려, 방향키로 한 번이라도 이동한 뒤엔 클릭 선택이 안 먹던
  버그를 자체 검증 중 발견·수정(메뉴를 다시 그리는 모든 지점에서
  클릭 재바인딩까지 함께 하는 `refresh()`로 통합).
- `tiptapToolbarAction(editor,wrapVal,lineVal)` — 기존 툴바 버튼의
  `data-mdwrap`/`data-mdline`(sc)·`data-semwrap`/`data-semline`(sem)
  속성값(index.html 그대로, 새 속성 없음)을 Tiptap 명령으로 매핑.
- `showEditorFallbackNote(afterEl)` — "조용한 안내 1줄"을 새 CSS
  클래스 없이 인라인 스타일+기존 디자인 토큰(`--ink-faint`)만으로
  삽입.

### 커밋① 자산 노트 (`assets.js`)

`a_notes` textarea+`an_previewToggle`(미리보기 토글, WYSIWYG라
불필요해짐)+`an_mdToolbar`+`an_mdPreview` → Tiptap 싱글턴. textarea는
`display:none`으로 숨기고 형제로 새 마운트 div(`an_tiptapMount`,
`.sc-md-editor .sc-md-content` 클래스 재사용)를 JS로 삽입. 저장은
`editor.storage.markdown.getMarkdown()`.

### 커밋② 수집함 폼·편집모달 (`scraps-form.js`)

`sc_text`(추가폼)·`sem_text`(수정모달) 두 contenteditable div를
Tiptap으로 전환. 두 div는 자식을 가질 수 있어(textarea와 달리) 새
마운트 없이 `el.contentEditable='false'`(JS만, HTML의
`contenteditable="true"` 속성 자체는 안 건드림)로 중첩
contenteditable 충돌만 없앤 뒤 Tiptap을 그 안에 직접 마운트 —
기존 `.sc-md-editor` 박스·`.slash-menu` 포지셔닝 wrapper를 그대로
재사용.

- **슬래시 커맨드 재구축**: 기존 `SC_SLASH`(10개 항목: h2/h3/ul/ol/
  quote/hr/bold/italic/code/codeblock) 리스트는 그대로 두고,
  `scSlashApplyTiptap(editor,key)`로 각 키를 Tiptap 명령
  (`toggleHeading`/`toggleBulletList`/`toggleBlockquote`/
  `setHorizontalRule`/`toggleBold` 등)에 매핑 — "기존 항목 동등
  이상" 충족(필터링·↑↓ 이동·Enter·클릭 선택 전부 재현, 위 utils.js
  함정 수정 후 정상).
- **ceRender 파이프라인 은퇴 여부**: 지시대로 utils.js의
  `ceRender`/`ceGetOffset`/`ceSetOffset`/`ceWrap`/`ceLine`/
  `ceFlushDebounced`/`ceRenderDebounced`/`ceCancelDebounced`/
  `ceRenderLine`/`ceInline` 전수 grep — **전부 참조 0이 아님**(삭제
  안 함). Tiptap **성공** 시엔 이 경로가 아예 붙지 않지만(기존
  `sc_text`/`sem_text`의 input/compositionstart/compositionend/
  paste/keydown/blur/focus 리스너 7종을
  `attachScTextFallbackListeners()`/`attachSemTextFallbackListeners()`
  함수로 묶어 **Tiptap 로드 실패 시에만 연결**), 그 폴백 경로 자체가
  이 함수들의 유일한 소비처로 남아 보존 필요. B-105/B-107이 하드닝한
  코드가 사장되지 않고 "CDN 장애 시 최후 방어선"으로 살아있는 구조.
- **로드 실패 폴백**: `initScTextEditor()`/`initSemTextEditor()`가
  `loadTiptapMods()` 실패를 감지하면 `contentEditable`을 원복하고
  폴백 리스너를 붙인 뒤 안내 1줄을 표시 — 기존 UI가 정확히 예전과
  동일하게 동작(esm.sh 차단 모킹으로 실측, 구 슬래시 메뉴·구 저장
  경로 전부 정상 확인).
- **모달 재사용 안전성**: `sem_text`는 여러 항목을 순서대로 열 때
  같은 Tiptap 인스턴스를 재사용(`setContent()`로 내용만 교체) —
  Tiptap이 이미 마운트된 뒤에 `innerHTML`을 덮어써 DOM을 깨뜨리는
  실수를 피하려고 "Tiptap 활성 여부"로 분기. 모달을 열자마자는
  기존 `ceRenderLine` 렌더로 즉시 내용을 보여주고(로딩 공백 없음),
  Tiptap 준비되면 같은 항목이 여전히 열려있는 경우에만 조용히
  교체(다른 항목으로 이미 넘어갔으면 무시).
- **B-108("1. " 리스트 접두 손상) 자연 해소 실증**: "1. first item"+
  Enter+"second item" 반복 타이핑 후 `getMarkdown()`에 손상 없음
  확인(ProseMirror 자체 리스트 모델이라 raw 문자열 슬라이스 방식의
  구조적 결함 자체가 없음) — B-103 도입 시 자연 해소된다던 예상이
  실측으로 확인됨.

### 검증(Playwright, 로컬 python UTF-8 서버, esm.sh 실접속+esm.sh 차단
모킹 양쪽) — 27개 체크 전부 통과

①한글 조합 5음절(기존 baseline 유지) ②모바일 390px 무overflow+타이핑
③(해당없음, 번들 크기는 PoC에서 이미 실측 완료) ④스타일(신규 클래스
0, 기존 클래스 재사용 확인) ⑤50KB+ sc_text 키 입력 2~3ms(임계
100ms 이내) ⑥⑨⑩ 라이브 숏컷(sc_text "## "→h2, sem_text "- "→ul)
+ Enter 리스트 이어짐 + 슬래시 커맨드 필터/↑↓/Enter/클릭 4가지 선택
경로 전부 ⑧XSS 3종(`<img onerror>`+`<svg onload>`+`<script>`+
`javascript:` 링크) 편집 표면·카드 렌더 양쪽 무실행 저장 왕복(Redis
모킹: guest 모드는 `state.js save()`가 의도적으로 로컬/Redis 저장을
건너뛰는 설계라 이 테스트만 `isGuestMode=false`로 인증 상태를
흉내내 실제 저장 경로 확인 — localStorage 즉시 반영+800ms 디바운스
후 POST 확인) 폴백 경로(구 슬래시 메뉴·구 저장 전부 정상) B-108
재현 불가 확인. 통합 스모크: 자산 노트+수집함 폼 에디터 동시
초기화(모듈 캐시 무충돌), 타입칩·태그·저장 전체 플로우, 카드 렌더,
매물탭(B-102) 무회귀. `node --check` 3파일 전부 통과.

**⚠️ 실기기 한글 확인은 사용자 몫** — 커맨드센터 채택 조건의 "실기기
한글 관문"이 이 배포에도 그대로 적용됨. Playwright/CDP의 조합 이벤트
시뮬레이션(`compositionstart`/`update`/`end` 합성 디스패치)은 실제
OS IME 흐름(여러 `compositionupdate`가 연쇄되는 실제 단어 조합 등)을
완전히 재현하지 못하는 한계를 PoC 때부터 일관되게 인정해왔음 —
자산 노트·수집함 폼·편집모달 3곳 모두 실기기(특히 모바일 한글
키보드)에서 직접 타이핑 확인 필요.

**→ B-103 2단계 ①② 완료·보고**. `index.html`/`nav.js`/`style.css`
전부 무접촉 확인. 커밋③(매물 메모 3곳, 동적 다중 인스턴스 필요한
`lst-memo-ta` 포함)은 검증 후 별도 지시 대기 — 여기서 중단.

---

## 2026-07-19 — B-106①: state 용량 상시 표시 + 80% 경고

- `save()`가 localStorage 저장용으로 이미 만든 JSON 문자열을
  `Blob.size`에 그대로 넘겨 UTF-8 바이트를 계측한다. 용량 표시를
  위한 추가 `JSON.stringify`는 없고, 같은 바이트 값을 localStorage
  실패 로그에도 재사용한다. 최초 로드 때만 가드 적용 후 현재 state를
  1회 직렬화해 초기 표시값을 만든다.
- 현재 바이트는 state 밖의 런타임 변수에만 두며 스키마는 바꾸지
  않았다. 동기화 칩 `title`에 `현재 저장 용량 … · 서버 4MB 상한의
  …%`를 상시 제공한다.
- `profile.js`가 프로필 모달 `.mbody` 끝에 `#pf_stateSize`를 JS로만
  마운트한다(`index.html` 무수정). 80% 이하는 기존 `.chip`의 조용한
  톤, **80% 초과**에서만 기존 `.chip-warn`과 "상한에 가까워요" 문구를
  사용한다. 자동 삭제·압축·저장 차단은 없다.
- Playwright 1440×900 + 390×844: 1.00MB→25%·일반,
  3.50MB→88%·경고, 4.10MB→103%·경고를 각각 정확한 target byte로
  확인했다. 두 뷰포트 모두 가로 overflow 0, 프로필 모달·동기화 칩
  표시 일치, 800ms 디바운스 POST 1회와 최종 동기화 `ok`를 확인했다.
  `save()` 호출당 stringify 계측도 세 구간 모두 정확히 1회였다.
  `node --check js/state.js`, `node --check js/profile.js`,
  `git diff --check` 통과.
- 실제 프로덕션 state는 인증된 개인 데이터라 자동 브라우저 저장소
  읽기를 수행하지 않았다. 배포 후 로그인된 프로필 모달에 표시되는
  수치를 사용자 확인받아 B-106② 정책 판단 근거로 이어간다.
- 손 A의 B-103 2-3 대상 `properties.js`·`utils.js`·`index.html`과
  `BACKLOG.md`는 무접촉.

---

## 2026-07-19 — B-103 2-3: 매물 메모 3곳 Tiptap 전환 — 에픽 코드 소진 (커밋 1개)

**B-103(서식 품질 에픽 2기) 코드 작업 마지막 커밋.** `f_memo`(매물
추가폼)·`em_memo`(매물 수정폼, 레거시 `state.properties` 경로)·
`lst-memo-ta`(단지 상세 매물 행, B-91 수정모드) 3곳 전부 Tiptap
전환. `properties.js` 단독 수정(+175/-2줄) — `index.html`은 이번에
락이 풀렸지만(B-53 완료) 결국 한 줄도 안 건드림, `utils.js`도 2-1·
2-2에서 이미 만든 공용 래퍼(`loadTiptapMods`/`showEditorFallbackNote`)
를 그대로 재사용해 무접촉. 손 B가 B-106①로 작업 중인 `state.js`·
`profile.js`도 무접촉(`git status`로 매 확인).

### "그림자 textarea" 패턴 — 2-1·2-2와 다른 새 설계 판단

f_memo/em_memo는 이전 커밋들의 "명시 분기"(`if(tiptapEditor){...}
else{...}`) 대신, **원래 textarea를 DOM에 남겨 숨기고 Tiptap의
`onUpdate`에서 매 변경마다 `textarea.value`에 마크다운을 계속
동기화**하는 방식을 택했다. 이유: `em_saveBtn`은
`document.getElementById('em_memo').value.trim()`을 그대로 읽고,
`data-lstsave` 저장 핸들러는 `[data-editfield]`를 전부 순회하는
**제너릭 루프**라 필드 종류를 모른다 — 후자를 분기 처리로 바꾸면
generic 루프에 memo만 특별 취급하는 코드를 심어야 해서 오히려
침습적이다. 그림자 패턴을 쓰면 저장 관련 기존 코드를
**한 줄도 안 건드리고** Tiptap이 얹힌다. B-92 자동채우기(`applyFill`·
`data-lstfill`)가 `.value=`로 직접 쓰는 지점만 추가로 Tiptap
인스턴스에도 `commands.setContent()`로 동기화해줬다.

### listing memo — Map 기반 멀티 인스턴스(유일하게 동시 2개+ 가능한 필드)

`cxListingEditMode`가 `Set`이라 매물 행 여러 개가 동시에 수정모드일
수 있음 — `listingMemoEditors = new Map<lid,{editor,mount}>()`로
관리. `renderCxListings()`가 호출될 때마다 `innerHTML`을 통째로
재조립하므로, 그 **시작 지점에서 반드시**
`destroyAllListingMemoEditors()`를 먼저 호출해 이전 렌더의 Tiptap
인스턴스를 `editor.destroy()`로 정리한다(DOM에서 사라진다고 JS
객체까지 GC되는 게 아님 — 안 하면 누수). 이후 `innerHTML` 재조립이
끝나면 그 시점에 `cxListingEditMode`에 남아있는 lid들만 다시
`initListingMemoEditor()`로 생성. 모달 "닫기"(저장/취소 없이 그냥
닫는 경우)에도 즉시 정리하는 안전망을 추가(`[data-close=
"complexDetailModal"]` 클릭 시 destroy — 안 해도 다음 아무 단지나
열면 자연 정리되지만 누수 창을 0으로 만듦).

**부수 발견·개선(스코프 내 처리)**: "행 A를 저장하면 아직 편집 중인
행 B의 미저장 타이핑이 사라진다"는 문제를 멀티 인스턴스 테스트
중 발견 — 원인은 `renderCxListings()`가 재렌더 시 **모든** 행의
textarea를 `l.memo`(마지막 저장값)로 다시 채우기 때문(Tiptap 도입
이전부터 있던 B-91 구조적 특성, 이번에 처음 노출된 게 아니라 "동시
2개 이상 열기" 시나리오가 이번에 처음 실측됨). 제너릭 저장/재렌더
로직 자체는 안 건드리고, **내가 새로 만든 destroy/init 함수 안에서만**
해결 — `destroyAllListingMemoEditors()`가 아직 편집모드인 행의
최신 마크다운을 `listingMemoPendingContent`에 잠깐 담아뒀다가
`initListingMemoEditor()`가 재생성 시 `l.memo` 대신 그 값을 쓴다.
Tiptap 전환이 기존보다 **더 안전한** 동작을 만든 케이스.

### memoPreviewToggle 3곳 처리

지시대로 WYSIWYG 전환 시 불필요해진 미리보기 토글 버튼·구 툴바를
Tiptap 성공 시 숨김(`display:none`) — 단 `memoPreviewToggle()` 함수·
클릭 핸들러 자체는 삭제하지 않고 남겨둠(로드 실패 폴백에서는 여전히
필요, 숨겨진 버튼은 클릭 자체가 불가능해 미리보기 토글은 성공
경로에서 자연히 도달 불가능해짐 — 함수 삭제 없이 안전하게 "제거"
효과).

### 검증(Playwright, 로컬 python UTF-8 서버, esm.sh 실접속+차단 모킹 양쪽) — 29개 체크 전부 통과

f_memo 라이브 헤딩+그림자 동기화+저장(v5 `saveAsComplexListing` 경로
확인 — **신규 매물은 `state.properties`가 아니라 `state.listings`로
라우팅됨을 재확인**, 첫 시도 때 잘못된 배열을 검증해 실패했다가
바로잡음)+폼 재오픈 시 초기화. em_memo 싱글턴 재사용(서로 다른
매물을 순서대로 열 때 내용 정확히 교체)+라이브 리스트+저장. listing
memo: **2개 행 동시 편집모드 진입 → Map 인스턴스 정확히 2개**,
서로 독립된 내용(교차 오염 없음), **한 행 저장 후에도 다른 행의
미저장 타이핑 보존**(위 개선 확인), 취소 시 Map 완전히 비워짐(누수
없음), destroy 후 재진입 시 중복 생성 없이 정상 재생성. B-92
자동채우기가 Tiptap 인스턴스와 그림자 textarea 양쪽에 일치 반영.
B-102① 읽기모드 메모 표시 무회귀. XSS 3종(`<img onerror>`+
`<svg onload>`+`<script>`+`javascript:` 링크) 편집 표면·카드 렌더
양쪽 무실행. 모바일 390px 무overflow. 저장 왕복(Redis 모킹, guest
모드는 저장을 건너뛰는 설계라 이 테스트만 인증 상태 흉내 — 2-2와
동일 방법론). 폴백 경로(esm.sh 차단 모킹): f_memo·listing memo
전부 기존 textarea 그대로 노출·저장 정상, Map엔 인스턴스 0개.
`node --check` 통과.

**→ B-103 2-3 완료·B-103 에픽 코드 작업 소진**. `index.html`/
`state.js`/`profile.js`/`BACKLOG.md` 전부 무접촉 확인. **에픽 종결
판정은 사용자 실기기 확인(한글 IME) 후 커맨드센터 몫** — 자산 노트·
수집함 폼/모달·매물 메모 3곳 총 6개 필드 전부 동일한 Playwright
시뮬레이션 한계(실제 다중 `compositionupdate` 연쇄 미검증)를 안고
있어, 최종 확인 없이는 "완료"로 자체 선언하지 않음.
...
## 2026-07-19 — B-104-2: 액션 AC3 명시적 그룹 + 콤팩트 행

B-104 1단계에서 추천하고 사용자가 채택한 AC3를 `js/actions.js`+
`style.css`만으로 구현. 미완료 액션을 `일반`·`매물준비`·`계약`
명시적 섹션으로 구획하고, 각 섹션 안은 체크·제목·분류·담당·마감·
액션을 한 줄에 놓는 AC2 콤팩트 행으로 바꿨다. `nav.js`·`index.html`·
`state.js`·`BACKLOG.md` 무접촉, 스키마 변경 없음.

- **정렬 정책**: 기존 `done → priority` 정렬을 먼저 확정하고 미완료
  결과에 category 그룹을 얹는다. 그룹 안은 기존 전역 priority 순서,
  ★ 맨 위로 semantics도 그대로다. 빈 그룹은 제목을 생략하고 미완료
  그룹은 접기 없이 전부 상시 노출. 빈 값/과거 미지원 category는
  일반 그룹으로 안전하게 표시한다.
- 완료 구획은 맨 아래 위치와 기존 접기 동작 유지. 추가폼·완료 토글·
  인라인 편집(편집 중 그룹 전폭)·담당/마감 뱃지·미완료만 마감 경과
  빨강·dday·B-93 추가 토스트·삭제를 모두 보존했다. 텍스트/담당은
  기존 `esc()` 렌더 경로를 유지한다.
- 390px은 그룹 내부 1열, 분류·담당·마감 칩을 제목 아래로 재배치.
  데스크톱 그룹 내부만 2열을 써 목록 길이를 줄인다.
- **B-95 관계**: 액션의 전역 행 우선 2열만 AC3 그룹별 2열로 대체.
  완료 `.done-body` 2열과 수집함 `.sc-list` 2열은 그대로 유지.

### 검증

Playwright 1440×900·390×844, 동일 24건(미완료 19·완료 5).
B-104-1 앱셸로 document 높이는 전후 900px·`maxScroll=0`; 액션 패널
`scrollHeight` 989→935px, 액션 목록 703→649px(54px·약 7.7% 감소).
그룹 3개/미완료 19건 전부 노출, 그룹 내 priority 순서, 완료 5건,
인라인 편집 취소·저장, 추가+B-93 토스트·완료·삭제, 마감 경과
뱃지(완료는 중립), 제목/담당 XSS 페이로드, `/api/state` GET/POST
모킹 저장 후 재로드 왕복 통과. 모바일 1열·메타칩 하단 배치·가로
넘침 0px·앱셸 내부 스크롤 추가폼 도달 확인. `node --check
js/actions.js`, `git diff --check` 통과.
...
## 2026-07-19 — B-111 커밋① fix: 편집 모달 레거시 렌더 잔존 제거 (커밋 1개, 커밋②는 style.css 대기)

**재발급 지시**(최초 발급분은 착수 흔적 없어 커맨드센터가 무효
판정, 이 지시서가 유효본). `js/scraps-form.js` 단독 수정(+12줄).

**근본원인**(커맨드센터가 실코드로 사전 특정, 이번 세션에서 실측
재현으로 확인): `openScEdit()`가 Tiptap 미초기화 상태일 때 빈 화면
방지용으로 `sem_text.innerHTML`을 `ceRenderLine`(레거시 B-105/B-107
렌더 파이프라인)으로 즉시 채운다. 그 직후(또는 로드 완료 후)
`initSemTextEditor()`가 `new mods.core.Editor({element:el,...})`로
Tiptap을 마운트하는데, **Tiptap의 Editor 생성자는 `element`에 이미
있는 자식을 지우지 않고 자기 `.ProseMirror`만 추가로 붙인다** — 그
결과 편집 불가(`contentEditable=false`인 부모 밑) 정적 사본이 새
에디터 위에 영구 잔존(저장 데이터 자체는 안전, 표시·선택 버그).
`esm.sh` 응답을 800ms 인위 지연시켜 "로딩 중엔 레거시 렌더가 보이고,
로드 완료 후 그게 안 지워지고 남는" 정확한 재현 시퀀스를 실측 확인.

**수정**: `initSemTextEditor()`의 `new mods.core.Editor(...)` 호출
직전에 `el.innerHTML='';`로 마운트 지점을 비운다. `initScTextEditor()`
(추가폼)도 점검 — 현재 코드 경로상 `sc_text`를 미리 채우는 지점이
없어 실증된 버그는 아니었지만, 같은 구조적 위험(Editor 생성자가
기존 자식을 안 지움)이 잠재하므로 동일한 안전망을 함께 추가(지시서의
"동일 패턴이면 함께 수정" 요구 반영).

**폴백 경로는 손대지 않음** — Tiptap 로드 실패/차단 시엔 이 레거시
렌더가 본편 UI이므로 제거 대상이 아니고, 그 경로는 원래대로
`el.innerHTML`을 그대로 유지한 채 동작.

### 검증(Playwright, 로컬 python UTF-8 서버) — 8개 전부 통과

편집모달 첫 열기(ProseMirror 1개만, 레거시 0개) 전체 선택 시 원문
전체 포함(레거시 사본이 선택 방해 안 함) 재열기(같은 항목) 다른
항목 연속 열기(내용 정확 교체+레거시 미잔존) **esm.sh 800ms 지연
모킹**: 로딩 중엔 레거시 렌더가 정상적으로 보임(빈 화면 방지 유지
확인)→로드 완료 후엔 완전히 사라지고 단일 에디터만 남음(수정 전엔
이 마지막 단계에서 실패했을 시나리오) sc_text(추가폼) 안전망 확인
폴백 경로(esm.sh 완전 차단) 레거시 렌더가 그대로 본편으로 정상
동작(무회귀). `node --check` 통과.

**→ 커밋① 완료·push 완료**(`c204c77`). **커밋②(✏️ 편집 힌트 제거,
`style.css`)는 아직 미착수** — 작업 중 `git status`로 손 B의
B-104-2b(`actions.js`+`style.css`) 미커밋 변경분을 확인, 지시서
조건("손 B push 확인 후 pull 받고 진행")에 따라 `style.css`
무접촉 유지하고 대기 중.

---

## 2026-07-19 — B-104-2b: 액션 그룹 와이드 칼럼 배치

B-104-2 이후 확정된 “전 탭 전체폭 프레임 + 탭별 폭 흡수 장치”
정책을 액션 탭에 적용. `js/actions.js`+`style.css`만 수정하고
`BACKLOG.md`는 무접촉.

- 1440px 이상에서 액션 탭의 문서형 max-width clamp를 해제하고
  일반·매물준비·계약을 가로 3칼럼으로 배치. 칼럼 내부는 행 1열이며,
  빈 그룹도 제목+`0`을 표시해 위치를 유지한다.
- 1439px 이하는 B-104-2의 세로 스택+그룹 내부 2열과 빈 그룹 생략을
  그대로 유지. 390px의 1열·메타칩 하단 배치도 무변경.
- 완료 구획은 칼럼 아래 전폭과 내부 2열 유지. 인라인 편집은 칼럼
  안에서 전폭 입력+보조 필드 줄바꿈으로 가로 넘침을 막았다.

### 검증

Playwright 1440×900·1920×1080·2560×1440·1439×900·390×844.
24건 목록 높이는 와이드 세 규격 모두 649→455px(`-194px`, 약
29.9%). wrap은 뷰포트와 동일한 1440/1920/2560px, document
`maxScroll=0`, 패널 내부 스크롤/overscroll contain, 가로 넘침 0.
빈 계약 그룹 `계약 0` 유지와 1439 이하 생략, 인라인 편집·추가·
B-93 토스트·완료·삭제·마감 뱃지·`/api/state` 모킹 저장 통과.
모바일 목록 높이 1482px 전후 동일. `node --check js/actions.js`,
`git diff --check` 통과.
...
## 2026-07-19 — B-112: 매물 추가 폼 열림 시 rail 하단 필터 칩 숨김 (커밋 1개)

사용자 실기기 스크린샷 확보 버그. 매물탭에서 추가 폼(`#form`)이
열린 채로 좌측 rail을 최하단까지 스크롤하면 필터 칩 바
(`#cxFilterBar`, 단지상태·매물상태·면적·세대수·노선)가 폼 아래에
노출됨 — DOM상 `#form`과 `#cxFilterBar`가 같은 `<section>`의
형제 요소이고, 데스크톱 기본 스타일에서 `#cxFilterBar`가 일반
흐름 가로 바(`display:flex`)라 폼이 열려 늘어난 만큼 rail이
길어지면서 그 아래 필터 바가 함께 스크롤 영역에 들어와 생긴
문제였다.

`style.css` 단독 수정(+5줄). `properties.js`는 손대지 않았다 —
`.form`이 열릴 때 항상 `open` 클래스를 받는 기존 상태 셀렉터를
그대로 활용해 순수 CSS 형제 셀렉터 `.form.open~#cxFilterBar
{display:none;}` 한 줄로 해결됨(JS 배선 불필요). DOM 제거가
아니라 `display` 토글이라 필터 칩의 `.on` 선택 상태는 그대로
보존된다.

- 특이도 계산상 이 새 규칙(클래스 2+아이디 1)이 모바일 지도뷰의
  기존 고정 오버레이 규칙(`.grid>section>#cxFilterBar`, 클래스
  1+아이디 1+태그 1)보다 항상 이겨, 모바일에서 폼이 하단 시트로
  열릴 때도 필터 바가 함께 숨는다 — 시트가 이미 필터 바를 시각적으로
  덮던 것과 동일한 결과라 회귀 아님.
- 리스트뷰에서는 JS(`syncListToolbar`)가 `#cxFilterBar`를
  `#cxListToolbar`로 실제 DOM 이동시켜 더 이상 `#form`의 형제가
  아니게 되므로 이 규칙이 적용되지 않는다 — 그쪽은 상단 sticky
  툴바라애초에 이 버그의 대상이 아님(의도된 범위 밖).
- `renderComplexes()`가 단지 0개일 때 `filterBar.style.display=
  'none'`을 인라인으로 강제하는 기존 로직(2316행)과는 인라인
  스타일이 항상 이겨 충돌 없음, 단지가 있을 때는 인라인을 `''`로
  되돌려(2323행) CSS 규칙이 정상 작동한다.

**검증**: Playwright(로컬 node UTF-8 정적 서버, guest 모드로
로그인 우회 후 테스트용 단지 1건 주입) 데스크톱 1440×900+모바일
390×844 총 13개 체크 전부 통과 — 폼 열기 전 필터 바 노출, 폼 열기
후 즉시 숨김, rail 최하단 스크롤 후에도 계속 숨김, 취소 버튼으로
닫은 후 필터 바 복원+기존 선택값(`.on` 클래스) 유지, 재열기 시
다시 숨김, 토글 버튼으로 닫은 후 복원, 리스트뷰 전환 무충돌,
모바일 시트 열림/닫힘 시 동일 동작. `node --check` 전체 통과
(코드 변경은 CSS뿐).

**→ B-112 완료·push 완료**(`5461c78`). `properties.js`/
`index.html`/`state.js`/`BACKLOG.md` 전부 무접촉. 손 B의
B-104-3(`scraps-render.js`+`style.css`) 미커밋 변경 없음을
`git status`로 확인 후 진행, 셀렉터 충돌 없음.
다음 손 A 작업은 B-113(별도 지시).

---

## 2026-07-19 — B-104-3: 수집함 카드뷰 기본화 + 리스트뷰 피드 1열

사용자 확정 정책 “전체폭 프레임 + 탭별 폭 흡수”를 수집함에 적용.
`js/scraps-render.js`+`style.css`만 제품 코드 수정하고
`BACKLOG.md`·스키마·`index.html`·`nav.js`·`scraps-form.js`는
무접촉.

### 뷰 의미 분리

- `scViewMode` 기본값을 `gallery`로 바꾸고 토글 표시도 JS 초기화
  시점에 동기화. 상태는 기존처럼 메모리에만 유지해 사용자가 리스트를
  고른 뒤 탭을 왕복해도 `renderScraps()`가 gallery로 강제 변경하지
  않는다.
- **카드뷰**: 제목·유형·상태·대표 사진·핵심 메타 최대 2개만 렌더.
  원문·더보기·상태 셀렉트·수정/자격확인/삭제 버튼은 제외하고,
  카드 클릭 또는 키보드 Enter/Space로 기존 `openScEdit()` 모달에
  진입. 사진이 없으면 유형별 색+아이콘 프리뷰를 표시한다.
- **리스트뷰**: 현행 상세 카드 렌더를 그대로 보존하되 전체폭 프레임
  안에서 `max-width:880px` 중앙 1열 피드로 배치. `renderMd()` 본문,
  B-97 원문↔서식 토글, 본문 더보기, 상태 셀렉트, 수정/자격확인/삭제,
  대표 사진·메타·태그가 모두 유지된다.
- 수집함의 문서형 wrap clamp를 해제. 카드 그리드는 1440px 이상
  `auto-fill minmax(380px,1fr)`로 폭에 따라 3열 이상, 1200~1439px
  고정 2열, 1199px 이하 1열이다.

### B-95 수집함 2열 은퇴

B-95는 동일한 상세 카드(원문+관리 기능)를 2열로 병치해 문서 높이를
줄였지만, 사용자가 기대한 “카드=빠른 탐색 / 리스트=한 건씩 읽기”
의미가 섞여 2열이어도 길고 산만하게 보였다. 따라서 수집함의
`.sc-list` 2열 규칙만 제거하고, 밀도 흡수 역할은 새 콤팩트 카드
그리드로 이전했다. 상세 리스트는 읽기 좋은 1열로 복귀. B-95 액션
2열과 B-104-2 AC3/와이드 칼럼 규칙은 무변경이다.

표시 안전성도 함께 보강 — 유형/상태 값이 사전 밖이면 안전한 fallback
CSS 클래스만 사용하고 라벨은 `esc()` 처리해 임의 클래스 삽입을
막았다. 제목·메타·태그는 기존 `esc()`, 상세 원문은 기존
`renderMd()` 경로를 유지하며 저장 필드 추가·변경은 없다.

### 검증

Playwright 36건 fixture(사진 12/무사진 24):

- 동일 1440px에서 기존 B-95 상세 2열 높이 **7,745px** → 새 카드
  3열 **3,717px**(`-4,028px`, **-52.0%**).
- 1440/1920/2560/1200/1439/768/390px 열 수
  **3/4/6/2/2/1/1**, desktop wrap 폭은 viewport와 동일.
- 전 규격 document `maxScroll=0`, 수집함 패널 `overflow-y:auto`+
  `overscroll-behavior:contain`, 가로 overflow 0.
- 두 뷰 왕복·다른 탭 왕복 후 선택 보존, 카드 클릭/Enter→기존 편집
  모달, 검색·유형 필터·최신/오래된 정렬·상태 변경 API 모킹 저장,
  B-97 원문↔서식 토글/renderMd 전부 통과.
- 제목 `<img onerror>`·태그 `<svg onload>` 페이로드가 텍스트로만
  표시되고 실행 안 됨. 카드 한 건당 핵심 메타 최대 2개, 카드 내부
  원문/버튼/셀렉트 0개 확인.
- `node --check js/scraps-render.js`, `git diff --check` 통과.

로컬·미커밋 캡처:
`mockups/b104-3-scraps-card-{1440,1920,2560,768,390}.png`,
`mockups/b104-3-scraps-list-{1440,390}.png`.

착수 직전 B-112가 `style.css`를 사용 중인 것을 확인해
...
## 2026-07-19 — B-104-4: 자산 탭 A2 요약·노트 / 등록부 2구역

사용자 확정 정책 “전체폭 프레임 + 탭별 폭 흡수”를 자산 탭에 적용.
`index.html`에 레이아웃용 래퍼만 최소 추가하고 `style.css`로
배치했다. 기존 자산 렌더 ID·이벤트를 그대로 재사용할 수 있어
`assets.js`·`nav.js`는 무접촉, 저장 스키마와 `BACKLOG.md`도
변경하지 않았다.

- 1440px 이상은 자산 탭 max-width clamp를 해제하고 좌측
  요약·목표·Tiptap 노트 / 우측 등록부 표를 `3fr:8fr` 2구역으로
  배치. 좌측 요약 카드는 세로 1열, 우측 표는 등록부의 긴 행을
  위해 남은 폭 전체를 사용한다.
- 1439px 이하는 래퍼가 일반 block 흐름으로 돌아가 기존
  요약→등록부→목표·노트 순서를 유지한다. 모바일 B-73 등록부
  2열 카드와 입력 크기도 그대로다.
- **sticky는 미채택**: 1440×900·24건 조건에서 좌측 콘텐츠 범위
  약 785px가 패널 가시 높이 775px보다 길었다. 이 상태에서 좌측
  전체를 고정하면 Tiptap 내부 스크롤과 중첩되고 하단 저장·안내
  영역 및 키보드 포커스 도달성이 나빠질 수 있다. 앱쉘 한 겹의
  자연 스크롤에서는 패널 `scrollTop=360`에 맞춰 좌측도 360px
  이동하고 에디터 포커스 시 브라우저 자동 스크롤이 정상 동작했다.

### 검증

Playwright 24건 fixture:

- 1440/1920/2560px에서 2구역·전체폭 확인. 실측 좌/우 폭은
  380/964, 497/1327, 672/1792px. 1439/390px는 기존 1열 순서.
- 전 규격 document `maxScroll=0`, 자산 패널
  `overflow-y:auto`+`overscroll-behavior:contain`, 가로 overflow 0.
- 실제 Tiptap CDN 에디터 포커스·편집·저장 API 왕복, 중첩 목록에서
  Home+Backspace로 한 단계 승격·Undo 복원·텍스트 보존 통과.
- 등록부 인라인 편집·행 추가/삭제·붙여넣기 임포트 모달과 저장 왕복,
  B-106 상태 용량 표시, 390px B-73 2열 카드·필드 라벨·44px 입력
  높이 통과.
- `node --check js/assets.js`, `node --check js/nav.js`,
  `git diff --check` 통과.

로컬·미커밋 캡처:
`mockups/b104-4-assets-zones-{1440,1920,2560}.png`,
`mockups/b104-4-assets-mobile-390.png`.
...
## 2026-07-19 — B-104-5: 대시보드 D2 콤팩트 커맨드센터

사용자 확정 정책 “전체폭 프레임 + 탭별 폭 흡수”의 마지막 조각을
대시보드에 적용했다. `style.css` 단독 수정(+17/-4줄).
기존 DOM 순서가 이미 여정→요약→Top3→안내라 CSS grid-area만으로
재배치할 수 있어 `nav.js`·`index.html`은 무접촉, 저장 스키마와
`BACKLOG.md`도 변경하지 않았다.

- 대시보드의 `max-width:clamp(1440px,60vw,1600px)`를 해제하고
  전체폭 프레임으로 전환.
- 1440px 이상에서 `#panel-dash.on`을 `4fr:3fr` 2열 그리드로
  구성했다. 여정 타임라인은 상단 전폭, 자산·매물 요약 카드 2장은
  좌측 같은 행, “지금 우선순위 액션 Top 3”는 우측 같은 행,
  수집함 안내 배너는 하단 전폭이다.
- 1439px 이하는 grid 규칙이 적용되지 않아 기존 block 흐름
  (여정→요약 카드→Top3→안내)으로 복귀한다. 390px의 요약 카드
  1열도 기존 모바일 규칙 그대로다.
- 렌더·상호작용 함수는 전부 그대로 유지:
  `renderJourney()`·`renderDashSummaries()`·`renderTop3()`와
  Top3 추가/완료/별표/삭제, `[data-goto]`, 안내 배너 배선 무변경.

### 검증

Playwright 실제 상태 fixture(마일스톤 4+오늘, 자산 6건, 단지 4/
매물 5건, 액션 7건):

- 1440/1920/2560px에서 좌측 요약/우측 Top3 폭
  **792/594, 1066/800, 1432/1074px**, 두 영역의 y 좌표 동일.
  wrap 폭은 각각 viewport와 정확히 동일하고 요약 카드 2장은 좌측
  내부에서 한 행을 유지했다.
- 1440×900에서 콘텐츠 하단 624px, 패널 하단 867px로 첫 화면에
  **243px 여유**. 와이드 세 규격 모두 패널 추가 스크롤 0,
  document `maxScroll=0`, 가로 overflow 0,
  `overflow-y:auto`+`overscroll-behavior:contain`.
- 1439px는 기존 세로 섹션 흐름+요약 카드 2열, 390px는 요약 카드
  1열. 모바일 document `maxScroll=0`, 대시 패널 내부
  `maxScroll=258px`, 가로 overflow 0.
- 여정 마일스톤·D-day와 자산/매물 요약 수치 렌더 확인. Top3
  추가→저장 POST, 완료→다음 항목 진입, 별표→맨 위 이동과 저장
  왕복 통과.
- Top3 더보기→액션, 요약 카드→자산/매물, 안내 배너→수집함 이동
  전부 통과. 모든 대상 패널 `scrollTop=0`, document 0으로
  B-104-1의 “전환 시 맨 위 초기화” 정책과 일치.
- 액션/마일스톤 표시의 기존 `esc()` 경로에 XSS 페이로드를 넣어
  DOM 삽입·실행 0 확인. `node --check js/nav.js`,
  `git diff --check` 통과.

로컬·미커밋 캡처:
`mockups/b104-5-dash-compact-{1440,1920,2560}.png`,
`mockups/b104-5-dash-compact-mobile-390.png`.

B-116의 제품·문서 커밋(`01057fa`/`bd62bb8`)을 최신화한 뒤
`properties.js`와 충돌 없이 마무리했다. **B-104 에픽은 코드
소진**이며, 최종 종결 판정은 사용자 대형 모니터 전 탭 확인 후다.
다음 손 B 작업은 B-114(별도 지시).

---

## 2026-07-19 — B-117: 매물 상세 사이드 패널 — 단지 상세 옆 확장 (커밋 1개)

사용자 결정("매물 상세는 단지 상세 모달 옆으로 더 열리는 형태")을
그대로 구현. `index.html`(+13줄)+`js/properties.js`(+110/-2줄)+
`style.css`(+26/-1줄) 수정, `state.js`·`BACKLOG.md` 무접촉·스키마
변경 없음. 손 B의 B-114(`actions.js`+`style.css`) 미커밋 변경과
`style.css` diff 겹침 0(매 커밋 전 확인).

### 구조

새 모달을 따로 만들지 않고 `#complexDetailModal` 안에 기존
`.box`(단지 상세)의 **형제로 먼저 오는** `#cxListingDetailBox`
(class `cx-listing-detail`)를 추가했다.

- **데스크톱**: `#complexDetailModal{justify-content:flex-end}`
  플렉스 그룹 안에서 DOM 순서만으로 단지 상세 왼쪽에 자연스럽게
  나란히 붙는다(폭 380px 고정, 높이 100vh). 별도 z-index 불필요 —
  형제 `.box`가 `z-index:auto`라 DOM 순서로만 위치가 정해진다.
- **모바일**: 옆 확장이 불가능해 `position:absolute;inset:0`으로
  같은 모달 안에서 전체 화면을 덮는 별도 시트로 스택한다. 로컬
  `z-index:1`로 형제 `.box`만 이기면 충분해, B-59가 이미 쓰는
  전역 1100 티어를 새로 침범하지 않는다(지시서의 "z-index 티어
  충돌 없게" 요구를 새 티어 신설이 아니라 기존 자식 스태킹
  컨텍스트 안에 머무는 방식으로 충족).
- 같은 모달의 배경 딤·열림 생명주기를 공유하므로 이중 딤(반투명
  배경 두 겹) 문제가 없다 — `.status-picker`처럼 완전히 독립된
  모달이었다면 겪었을 문제.

### 표시(읽기 전용) + 편집 신설 금지

동호수·전용면적·보증금·관리비(`mgmtFeeCaption()` 텍스트만, 인터랙티브
tri-state 컨트롤 아님)·상태·대표 여부·URL(`safeUrl()`)·메모
(`renderMd()`)·안전 체크 9항목 전체·`lastCheckedAt`을 전부 표시한다.

- 안전 체크는 새 `safetyReadOnlyHTML()`로 렌더 — 기존 편집용
  `safetySectionHTML()`의 select/input과 완전히 분리된 읽기 전용
  버전(상태 라벨+메모+출처+확인일만 텍스트). 같은 `SAFETY_ITEMS`/
  `SAFETY_STATUS_LABEL`(state.js)만 재사용, 새 데이터 형태 없음.
- 새 `daysAgoLabel()`로 `lastCheckedAt`을 "n일 전 확인"으로 표시 —
  "게시중 확인" 버튼이 기록해온 값이 처음으로 사용자에게 보이는
  지점이라 B-115(게시중 확인 버튼 가치 재판단) 재평가의 실측
  근거가 될 수 있다.
- 패널의 "수정" 버튼은 새 폼을 만들지 않고 기존
  `cxListingEditMode.add(lid)`+`renderCxListings()`(B-91 인라인
  수정모드)를 그대로 호출한다 — 패널은 닫히고 해당 행이
  `scrollIntoView`로 포커스된다.

### 동기화·클릭 위임·닫기 경로

- **동기화는 단일 지점**: 대표매물 설정·게시중 확인·사라짐·
  가격변동·삭제·tri-state·안전체크·인라인 수정 저장/취소 등
  모든 매물 데이터 변경 경로가 공통으로 거치는 `renderCxListings()`
  끝에 `renderListingDetailPanel()` 한 줄만 추가했다. 개별
  핸들러마다 동기화 호출을 따로 심을 필요가 없었고, 보던 매물이
  삭제되면 "찾으면 렌더, 없으면 닫기" 내부 분기로 패널이 자동으로
  닫힌다(별도 삭제 감지 코드 없이 공짜로 얻은 안전망).
- **행 클릭 vs 버튼 간섭 없음**: `#cxDetailListings` 위임 클릭에서
  `e.target.closest('a,button,input,select,textarea')`면 무시한다.
  수정·삭제·URL·안전체크 select 등 기존 상호작용 요소가 전부 이
  태그들이라 data-속성 화이트리스트를 따로 나열할 필요가 없었다.
  기존 핸들러들과 `stopPropagation` 없이 병행해도 각자 독립적으로
  `closest` 매칭 후 이르게 return하는 기존 패턴이라 충돌하지
  않는다.
- **닫기 경로 3종 전부 패널까지 정리**: `[data-close]` 제네릭
  핸들러·`.modal` 배경 딤 클릭 제네릭 핸들러·B-21 드래그다운
  `finish()` 세 곳 모두 `complexDetailModal`을 닫을 때
  `closeListingDetail()`도 같이 호출하도록 확장했다.
  `openComplexDetail()`도 진입 시 이전 패널을 리셋해 다른 단지로
  전환할 때 잔존하지 않는다.
- B-21 드래그다운은 `modal.querySelector('.box')`(첫 `.box`만)에
  바인딩돼 있고 새 패널은 `class="box"`를 쓰지 않아 애초에 접점이
  없다 — 모바일에서 패널이 열려있으면 화면을 완전히 덮어 아래
  `.box`가 터치 이벤트 자체를 받지 못한다(구조적으로 충돌 불가능,
  코드로 막은 게 아니라 레이어 순서상 원천 차단).

**검증**: Playwright(로컬 node UTF-8 정적 서버, `window.naver` 최소
스텁으로 단지 상세 지도까지 정상 초기화, guest 세션) 데스크톱
1440×900+모바일 390×844 38개 체크 전부 통과:

- 행 클릭→패널 오픈+전체 필드(면적·보증금·"관리비 15만원"·상태·
  대표여부·"3일 전 확인"·안전체크 9항목 라벨 전부+상태라벨(문제없음/
  주의)+메모/출처) 정확 표시.
- 패널이 단지 상세 박스 바로 왼쪽에 인접(gap 0)+폭 정확히 380px.
- 다른 매물 클릭 시 스택 없이 패널 1개만 존재, 내용 교체 확인.
- 정상 URL은 `safeUrl` 통과한 그대로 링크로, `javascript:` URL은
  링크 자체가 렌더 안 됨.
- 메모 XSS(`<img onerror>`) — 스크립트 미실행+렌더 결과에 `onerror`
  속성 자체가 없음(DOMPurify 정화 확인)+마크다운 굵게는 정상 렌더.
- 행의 기존 버튼(수정·게시중 확인) 클릭이 패널을 새로 열거나
  잘못 닫지 않고 각자 동작 정상 수행("게시중 확인" 클릭 전후
  `lastCheckedAt` 값 변화로 실제 동작까지 확인).
- 패널의 "수정"→기존 인라인 수정모드 진입(`cxListingEditMode`에
  추가됨)+패널 닫힘.
- 닫기 3종 각각 정확한 범위로 닫힘 확인: 뒤로=패널만/배경 클릭=
  패널+단지 상세 둘 다(+`cxListingDetailId` 리셋까지)/상단 닫기
  버튼=패널+단지 상세 둘 다.
- 멀티 매물 왕복(A→닫기→B→닫기→A) 후에도 항상 최신 클릭 매물로
  정확히 표시.
- 보던 매물이 삭제되면 패널 자동 닫힘.
- 모바일: 행 탭→패널이 뷰포트 100% 커버하는 전체 화면 시트로
  스택+단지 상세 시트는 그 아래 열린 채 유지+뒤로 탭으로 복귀.

`node --check` 통과.

**→ B-117 완료·push 완료**(`932b794`). 손 B의 B-114
(`actions.js`+`style.css`)와 파일 충돌 없음. 다음 손 A 작업은
B-118(별도 지시).

---

## 2026-07-19 — B-114: 액션 순서 변경 — 드래그앤드롭 + 우선순위

사용자 결정("드래그앤드롭 채택+우선순위 체계 유지")에 따라
`js/actions.js`(+129/-5줄)+`style.css`(+25/-2줄)를 수정했다.
신규 order 필드를 만들지 않고 기존 `actions[].priority`만
재번호화해 스키마 변경 없음. `BACKLOG.md` 무접촉. 손 A의 B-117이
`style.css`를 먼저 수정하는 동안 파일 락을 지켜 로컬 CSS
오버라이드로 검증했고, B-117 제품/문서 커밋
`932b794`/`9e145ca`가 push된 뒤 최신 origin/master에서 비중첩
상태를 확인해 실제 CSS를 반영했다.

### 순서 모델·★ 정책

- 미완료 항목은 기존 priority 오름차순으로 읽고, drop 결과를
  1..N으로 재번호화한다. 완료 항목의 priority는 건드리지 않아
  기존 done→priority 정렬을 유지한다.
- 미완료 중 priority 최솟값 하나가 기존 ★ 의미인 "맨 위 고정"
  행이다. 고정 행 자체는 drag/▲▼를 비활성화하고 어떤 이동도 그
  위에 삽입되지 않게 정규화한다. 고정 행을 옮기려면 다른 행의 ★를
  먼저 눌러 고정 대상을 교체해야 한다.
- ★는 그룹별 1개가 아니라 전역 1개다. 어느 칼럼에 있든 전체
  우선순위 1위이고 해당 그룹에서도 첫 행을 유지한다. 이동 직후
  `renderTop3()`도 호출해 대시보드 우선순위와 동기화한다.
- 검색/분류 필터 중에는 숨은 행 사이의 삽입 위치를 사용자가 확인할
  수 없으므로 순서 변경 UI를 잠근다. 이는 데이터 손실 방지 정책이며
  필터 해제 즉시 다시 활성화된다.

### 데스크톱 drag·칼럼 간 category 변경

- 미완료 행 왼쪽의 `⋮⋮` 전용 손잡이에서만 drag를 시작한다.
  체크박스·텍스트·수정/★/삭제 버튼·인라인 편집 input/select는
  draggable 대상이 아니어서 기존 상호작용과 분리된다.
- 같은 칼럼 안의 앞/뒤 삽입을 지원하고, 다른
  일반/매물준비/계약 칼럼으로 drop하면 기존 `category`도 함께
  변경한다. 빈 칼럼은 평소 AC3 정책대로 숨기되 drag 중에는
  제목+최소 drop 영역을 잠시 표시해 칼럼 간 이동을 막지 않는다.
- drag 행 반투명, 앞/뒤 삽입선, 대상 칼럼 wash 배경만 기존 money
  토큰으로 표시한다. 액션 패널 위/아래 48px에서는 내부
  `scrollTop`을 조금씩 이동해 긴 목록에서도 drop할 수 있다.
- 성공 시 기존 B-93 `toast()`로 같은 그룹이면 "순서를 바꿨어요",
  칼럼을 넘으면 "분류와 순서를 바꿨어요"를 표시한다.

### 모바일 폴백·접근성

899px 이하에서는 터치 HTML5 drag 손잡이를 숨기고 각 행 왼쪽에
▲▼ 버튼을 배치했다. 같은 그룹 안에서 한 칸씩 이동하며 그룹 경계와
★ 고정 행은 disabled다. 따라서 390px에서도 반드시 순서를 바꿀 수
있다. 데스크톱 손잡이는 키보드 포커스 후 ArrowUp/ArrowDown으로도
동일한 이동 함수를 호출한다. 완료 구획에는 손잡이와 ▲▼를 모두
렌더하지 않는다.

**검증**: Playwright 실제 앱+Redis API mock, 미완료 12건/완료 2건.

- 1440×900: AC3 3칼럼+데스크톱 손잡이, 같은 그룹 재정렬, 칼럼 간
  일반→계약 이동과 category 변경, `actions[].priority` 1..N
  재번호화, 저장 POST 후 새로고침 순서/분류 유지.
- ★를 다른 칼럼 항목으로 교체한 뒤 새 ★ 행은 이동 불가, 기존 ★
  행은 이동 가능, 새 ★가 계속 전역 최솟값인 것을 확인.
- 검색 중 손잡이/▲▼ 잠금, 인라인 편집 시 행이 기존 전폭 폼으로
  교체되고 저장 왕복, 액션 추가→완료→삭제, 담당·마감/경과 뱃지,
  완료 구획 drag 제외, 토스트 표시를 확인.
- 1440×500: 액션 패널 내부 `maxScroll=226px`,
  `panel.scrollTop=226`, document `maxScroll=0`; 추가 input은
  draggable=false. 앱쉘 스크롤과 drag 시작 영역 분리 확인.
- 390×844: 1칼럼 복귀, drag 손잡이 숨김, ▲▼ 표시와 아래→위 왕복,
  ★/경계 disabled, document `maxScroll=0`, 가로 overflow 0,
  `overflow-y:auto`+`overscroll-behavior:contain`.
- 저장 POST 10회, 브라우저 page error 0.
...
## 2026-07-19 — 코드·문서 효율 감사 (B-119, 코드 무변경)

`BACKLOG.md` B-119. 커맨드센터 사전 스캔(properties.js 3,696줄=
전체 JS 55%, HANDOFF.md 2,035줄 등)을 검증·심화했다. 4개 스코프를
읽기 전용 하위 조사(병렬 3건 + 직접 분석 1건)로 나눠 진행했고,
**코드·문서는 한 글자도 바꾸지 않았다** — 이 커밋은 진단 기록뿐이다.
BACKLOG.md는 손대지 않음(커맨드센터 전용).

### 발견 요약 (심각도순)

| 심각도 | 항목 | 규모 | 근거 |
|---|---|---|---|
| 🟡 | HANDOFF.md 매 세션 로드 비용 | 47개 핸드오프 엔트리·2,162줄·124,617바이트, HISTORY.md와 내용 상당 중복 | 아래 ④ |
| 🟡 | `properties.js` 죽은 CSS 변수 생산 코드 | `--nav-h`/`--topbar-h`/`--overlay-top` 3개 변수를 스크롤·리사이즈마다 계산·`setProperty` — `style.css` 소비처 0건(grep 재확인) | `properties.js:170-193`(생산), `style.css` 전체 0매치 |
| 🟡 | `properties.js` 55%·155개 DOM ID 결합 + `claudeAPI` 역방향 의존 | `claudeAPI`(565행)를 로드순서상 **먼저** 로드되는 `actions.js`·`scraps-form.js`·`scraps-import.js`가 참조 | `properties.js:565` 정의, `js/actions.js`·`js/assets.js`·`js/scraps-form.js`·`js/scraps-import.js`·`js/ai.js`에서 호출 |
| 🟡 | `js/scraps-form.js` Tiptap 마운트 불안정 구간 | 이틀간 6개 커밋 연속 수정(`1ac7817`→`93d7ab3`→`983dd17`"B-105 회귀 핫픽스"→`5ac5454`→`cc0f6e0`→`c204c77`) | `git log --since="2026-07-17" -- js/scraps-form.js` |
| 🟡 | 레거시 `state.properties[]` 활성 수정 경로 생존 | 직접 참조 23곳(신규 스키마 92곳의 1/4), CRUD 함수 15개+ 그대로 동작(추가/수정/삭제/상태변경) | `properties.js:1009,1127,1205,1334,1339,1347,1367` 등, `profile.js:126-133`, `state.js:404` |
| 낮음 | `showStatusPicker`↔`showCxStatusPicker` 중복 | 거의 동일한 플로팅 메뉴 로직 46줄, **의도적 복붙**(주석에 명시) | `properties.js:1367-1385`(레거시)·`2396-2426`(신규), 주석 `2396-2398` |
| 낮음 | 매물↔단지 마이그레이션 프리뷰 죽은 코드 | `migBuildRows`+`renderMigPreview`+`migApply`+`migInjectUI` 약 160줄 — 여는 진입 버튼 자체가 제거돼 도달 불가(개발자 자인 주석) | `properties.js:2202-2343,2369-2389`, 주석 `2365-2368`(B-68) |
| 낮음 | 고아 함수 | `assetTotal()` 1줄, 정의 외 참조 0건 | `js/nav.js:166` |
| 낮음 | 미사용 CSS 셀렉터 | 전체 479개 클래스 중 25개(≈5%) 미참조 확정(동적 생성 18개는 오탐 제외 처리) | `style.css` 다수 지점(아래 상세) |
| 낮음 | `alert()` vs `toast()` 비일관 | 공용 `toast()` 있음에도 6곳이 네이티브 `alert()` 유지 | `properties.js:392,401,417,423,1416,1913` |
| 낮음 | `CLAUDE.md` 파일 규모 서술 구식 | `index.html` "~670줄" 서술 vs 실제 1,042줄, `style.css` "~700줄" 서술 vs 실제 1,509줄(2배 이상) | `CLAUDE.md:22-23` |
| 확인결과 없음 | B-95 vs B-104 CSS 중복/덮어쓰기 | 각 후속 커밋이 이전 규칙을 주석과 함께 명시적으로 대체 — 죽은 규칙 없음 | `style.css:412-434,897-899` 등, 커밋 `06d7b09`→`70d8dab`→`4c45887`→`3d8b6aa` |
| 확인결과 없음 | 미디어쿼리 브레이크포인트(900/1200/1439/1440) | 전수 확인 결과 경계 어긋남·갭 없음, JS(`nav.js:3`,`properties.js:144`) 값과 CSS 완전 일치 | `style.css` `@media` 전수 grep |
| 확인결과 없음 | `ceRender` "레거시 렌더 폴백" | 이름은 실존하나 의미가 다름(contentEditable 라이브 렌더, 정상 활성 코드). 진짜 폴백은 `attach*FallbackListeners`(Tiptap 로드 실패시만 활성, 약 220줄) — 죽은 코드 아님 | `utils.js:52-186`, `scraps-form.js:40,476` |

### ① 파일 비대·분할 — `properties.js`

3,696줄을 기능 블록 12개로 지도화했다(지도 렌더 49-234행, 임장
루트 235-476행, AI 연동+네이버 파싱 564-745행, 레거시 매물 CRUD
745-1401행, 가중치+AI평가 1402-1547행, 내보내기/임포트/마이그레이션
1548-2392행, **단지 카드+상세(최대 단일 블록, 약 1,055줄) 2392-3447행**,
매물 상세 패널 3447-3604행, 바텀시트/리사이즈 3604-3696행). `document.
getElementById` 고유 155개로 `index.html`과 강결합, 외부 전역
20개+ 참조.

**규칙 충돌**: "새 JS 파일 금지" 규칙을 지키는 한 분할 불가 —
파일 최상단 목차 주석 추가 정도만 가능(리스크 0, 즉시 적용
가능). 규칙을 완화한다면 절단선 후보:

| 안 | 절단선 | 장점 | 단점 |
|---|---|---|---|
| A. 2분할 | 1~1547 / 1548~3696 | 배너 주석 경계와 일치, 순방향 호출만(안전) | 1,547/2,149줄로 여전히 불균형 |
| B. 3분할 | 1~1547 / 1548~2392 / 2392~3696 | 실사용 "단지 시스템"(3부)과 대부분 비활성인 임포트·마이그레이션(2부)을 분리 | 파일 3개 추가로 규칙 명백 위반, `claudeAPI` 역방향 결합은 미해결 |
| C. 규칙 유지 | 분할 없음 | 즉시 적용, 리스크 0 | 3,696줄·155개 DOM 결합 근본 문제 그대로 |

로드 순서(`state.js`/`utils.js` 이후, `boot.js` 이전)만 지키면
두 파트 분리 자체는 순환 의존 없이 가능함을 확인했으나, `claudeAPI`
가 어느 파트에 남아도 로드순서상 더 앞선 4개 파일의 역방향 결합은
해소되지 않는다(별도 이관 작업 필요, 이번 범위 밖). **최종 판단은
사용자 몫.**

### ② 죽은 코드·중복 — 상세

요약 표 참고. 추가로 확인된 것: `toast()`·`openModal`/`closeModal`
은 이미 `utils.js` 단일 정의로 통일돼 있고(재구현 0건), `dday()`
(`nav.js:153`)와 `actDaysUntilDue()`(`actions.js:40`) 중복은
**이번 감사 기간 이전(B-30/B-56, 07-13)부터 존재하는 기존 부채**
이며 `actions.js:35-39` 주석이 "nav.js 무접촉 대상이라 의도적으로
분리 유지"라고 명시해 신규 회귀가 아님을 확인했다.

미사용 CSS 25개 상세: `.side`·`.scard`(구 설정카드), `.task`/`.box`/
`.tx`(구 대시보드 체크리스트), `.arow`/`.al`/`.aname`/`.ahint`/`.av`/
`.atotal`/`.tl`/`.tv`(구 자산 합계 행 — 현재는 `.regrow`/`.rcell`
계열로 대체 확인), `.c-top-left`, `.c-line`, `.og-info`/`.og-title`/
`.og-desc`, `.pam-header`/`.pam-arrow`/`.pam-list`/`.pam-item`/
`.pam-high`("pam" 문자열 자체가 코드베이스 전역 0건), `.slash-desc`/
`.slash-none`, `.split-placeholder`, `.atab.soon`. `mockups/`
디렉터리는 검색 대상에서 제외했음(별도 확인 필요, 미확인 표시).

### ③ 손 A/B 최근 2일 산출물 교차 검토

CSS 중복/미디어쿼리는 문제없음(요약 표 참고). 실질적 발견은 위
요약 표의 `properties.js` 죽은 CSS 변수 생산 코드 — B-53(`13a4b37`)
이 `--topbar-h`/`--nav-h`/`--overlay-top`을 소비하던 CSS 규칙을
전부 `--app-top-h`/`--active-panel-top` 체계로 교체했지만 `properties.
js`의 생산 코드는 "파일 락 때문에" 그대로 남겨뒀다고 **손 B 스스로
`HISTORY.md:4563-4568`에 기록**했고, 이후 B-104-1·B-98·B-118까지
계속 수정되면서도 정리되지 않았다. 매 스크롤 프레임마다 무의미한
`getBoundingClientRect`/`getComputedStyle`/`setProperty` 호출이
발생하는 성능 낭비이며 `BACKLOG.md`에 별도 항목으로 등록돼 있지
않아 추적이 누락된 상태다.

### ④ 토큰 효율 — 매 세션 로드 문서

- **`HANDOFF.md`**: 47개 핸드오프 엔트리, 2,162줄, 124,617바이트.
  최신 5개 엔트리만 해도 417줄(26,507바이트)이라, 나머지 42개
  (2026-07-18 B-84까지 소급)가 전체 분량의 약 79%를 차지한다.
  샘플 대조 결과(B-90) `HANDOFF.md`의 진단 서술과 `HISTORY.md:3022-
  3101`의 해당 커밋 기록이 실질적으로 같은 내용을 담고 있어, 오래된
  엔트리는 `git log`/`HISTORY.md`로 이미 보존되는 정보의 중복
  로드다. **제안(실행 안 함)**: `HANDOFF.md`는 최신 N개(예: 진행중
  1개+직전 완료 4~5개)만 유지하고, 그 이전 엔트리는 잘라내되
  git 이력에는 그대로 남으므로 정보 손실이 아니다 — 로테이션 기준·
  N값은 사용자 결정 필요.
- **`CLAUDE.md`**: 서술 구식 목록은 요약 표 참고(`index.html`/
  `style.css` 줄수). 세션 시작마다 로드되는 문서이므로 정확도가
  체감 신뢰도에 영향을 준다.
- **`HISTORY.md`**: `AGENTS.md`가 이미 "과거 맥락 필요할 때만
  해당 구간 읽기"로 규정해 매 세션 강제 로드 대상이 아님(6,010줄,
  이번 커밋 포함) — 별도 아카이빙 조치 불필요로 판단.

### 결정 필요 항목 (사용자 몫, 별도 표기)

| 항목 | 옵션 |
|---|---|
| `properties.js` 분할 허용 여부 | A(2분할)/B(3분할, 규칙 완화 필요)/C(분할 없음, 주석만 보강) |
| `claudeAPI` 위치 이전 여부 | 현행 유지(properties.js) / utils.js·ai.js로 이관(역방향 결합 해소) |
| 마이그레이션 프리뷰 죽은 코드(약 160줄) | B-05까지 존치 / 즉시 삭제 |
| `HANDOFF.md` 로테이션 정책 | 유지 개수(N) 및 절삭 시점 확정 |
| `CLAUDE.md` 파일 규모 서술 갱신 | 승인 시 다음 세션에서 1줄 수정 반영 |

### 착수 순서 제안 (리스크 낮은 것부터)

1. `CLAUDE.md` 파일 규모 서술 갱신(리스크 0, 문서 정확성)
2. `HANDOFF.md` 로테이션 정책 확정·적용(리스크 0, 토큰 절감 최대)
3. `properties.js` 죽은 CSS 변수 생산 코드(170-193행) 삭제(리스크
   낮음 — 소비처 0건 확인됨, 손 B 파일 락 해제 여부만 확인)
4. `scraps-form.js` Tiptap 마운트 로직 — 다음 변경 전 회귀 테스트
   케이스 확보(회귀 이력 있는 구간)
5. `showStatusPicker`/`showCxStatusPicker` 통합 검토(낮은 우선순위)
6. `properties.js` 분할 여부 — 사용자 결정 후 착수(B-05·claudeAPI
   이관과 연계 판단 필요)
...
## 2026-07-19 — B-122 지침 강화: 확립 원칙 CLAUDE.md/AGENTS.md 명문화

B-41(임장 노트)보다 먼저 발급된 문서 전용 지시. 최근 세션들에서
이미 실무로 확립됐지만 문서화되지 않았던 원칙 8개를 추가·보강만
했다(기존 구조·규칙 삭제 없음, 코드 무변경, 1커밋 `d530796`).

**CLAUDE.md 4건**:
1. `## UI/레이아웃 정책` 신설 — 전 탭 전체폭 프레임+탭별 폭 흡수
   장치(매물=지도·액션=그룹 칼럼·수집함=카드 그리드·자산=2구역·
   대시=콤팩트 행, 2026-07-19 확정), 중앙 고정폭 신설 금지.
2. 같은 섹션에 접기 지양 원칙(2026-07-18 확정) — 명시적 버튼·모드
   전환, 말줄임(ellipsis)은 접기와 구분해 허용으로 명시.
3. `## 핵심 패턴`에 **에디터(Tiptap) 표준** 항목 신설 —
   `loadTiptapMods()`(esm.sh 동적 import)+`buildListBackspaceFix`+
   `buildTiptapPlaceholder` 확장 연결, 로드 실패 시 `.catch(()=>null)`
   →기존 입력 폴백+`showEditorFallbackNote` 안내, 저장은 마크다운
   그대로·읽기는 `renderMd`+DOMPurify. "새 필드에 일부만 적용 금지"
   원칙과 현재 6필드(자산 노트 1·수집함 2·매물 메모 3) 완전 적용
   전례를 실코드 grep으로 재확인 후 기입(properties.js 3곳+
   assets.js 1곳+scraps-form.js 2곳=6, 정확히 일치).
4. 파일 구조 표의 `style.css` 줄수를 B-121 CSS 정리 이후 실측치로
   재정정(~1,510→~1,460줄). `index.html`은 무변경이라 유지.

**AGENTS.md 4건**(`## 검증 표준` 신설):
5. 실기기 관문 — 한글 IME·safe-area·실키보드·PWA PTR 등 자동화
   한계 항목은 "사용자 실기기 확인"을 완료 조건으로 HANDOFF에 명시
   (B-31 인수 사례가 근거).
6. 렌더 타이밍 변경(디바운스·지연·비동기화) 시 "지연 중 동기
   상호작용" 케이스(연타 직후 Enter/단축키/붙여넣기) 검증 필수 —
   `utils.js`의 `ceFlushDebounced()`(모든 동기 진입점 앞에서 예약된
   렌더를 즉시 flush)가 실제 적용 패턴(B-107 회귀 교훈).
7. CSS 변수 도입·폐기 시 소비자 전수 grep 확인 — B-121에서 발견한
   `--nav-h`/`--topbar-h`/`--overlay-top` 무소비 변수(B-53에서
   소비 CSS만 교체하고 생산 JS는 남겨둔 사례)가 교훈.
8. 파일 락 프로토콜 명문화 — 착수·커밋 직전 `git status`, 본인
   파일만 명시적 `git add`(`-A`/`.` 금지), 상대 미커밋 변경은
   diff가 안 겹쳐도 무접촉·무스테이징(B-84·B-118·B-31에서 실제
   반복 적용된 패턴을 규칙으로 승격).

**검증**: 추가 서술에 등장하는 함수명·파일명·수치를 실코드 대조로
전수 재확인 — `ceFlushDebounced`(`utils.js:157`)·`showEditorFallbackNote`
(`utils.js:463`) 실존 확인, `style.css` 1,462줄·`index.html` 1,042줄
실측, 6필드 호출부 grep 일치 확인. `git diff` 전체를 재검토해 삭제
없이 추가·수치정정만 있음을 확인(AGENTS.md +7줄, CLAUDE.md +11/-1줄).
BACKLOG.md 무접촉.

**→ B-122 완료·push 완료(`d530796`)**. 다음은 기발급 지시서대로
B-41(임장 노트) 착수 예정이었으나, `BACKLOG.md`에 "착수 전
...
## 2026-07-20 — B-43 매물 스냅샷 — 가격·상태 변동 자동 기록

커맨드센터가 확정한 스펙으로 착수. "자동 기록"이 "판정 금지 원칙"과
충돌하지 않는지는 커맨드센터가 사전 검토 완료(기록은 판정이 아님).
손 B 부재로 파일 락 제약 없이 진행, 2커밋(`59ddcde`/`41de716`)
분리.

### 스키마 (`js/state.js`)

`listings[].history[] = [{at(ISO), deposit, listingStatus,
source}]`. `source`는 변경 경로 식별자로 `'create'|'edit'|'check'|
'gone'|'price'|'merge'` 6종을 사용(스펙 원문의 `'promote'`는 아래
"경로 조사" 참고 — 실제로는 불필요 판단). `applyGuards()`는
`guardArr(l.history,[],...)`로 **배열 형태만** 보정한다 — 누락 시
빈 배열이 기본값이고, **과거 변경분을 소급 생성하지 않는다**(추정
기록 금지 원칙 그대로 준수). 표시용 `HISTORY_SOURCE_LABEL`(2번째
커밋)은 `SAFETY_STATUS_LABEL`과 동일하게 state.js·properties.js가
공유하는 상수로 신설.

### 변경 경로 전수 조사 및 배선 (`js/properties.js`)

스펙이 나열한 시나리오("신규 생성·수정모드 저장·게시중 확인/사라짐
처리/가격변동 기록 버튼·parseNaver 자동채우기·승격·병합 이관")를
`grep`으로 전수 추적해 실제 코드 지점 8곳을 확인:

| # | 코드 지점 | source | 기록 방식 |
|---|---|---|---|
| 1 | `saveAsComplexListing()`(ADD 폼 저장, ~1253행) | create | 무조건 |
| 2 | TSV 벌크 임포트 매물 생성(~2084행) | create | 무조건 |
| 3 | 레거시 마이그레이션 위저드(~2261행, B-05 대기 코드지만 포함) | create | 무조건 |
| 4 | 수정모드 "저장"(`data-lstsave`) | edit | 값 변경시만 |
| 5 | "게시중 확인"(`data-lact="check"`) | check | 값 변경시만 |
| 6 | "사라짐 처리"(`data-lact="gone"`) | gone | 값 변경시만 |
| 7 | "가격변동 기록"(`data-lact="price"`) | price | 값 변경시만 |
| 8 | B-118 병합 이관(`cxMergeExecute` 내 `l.complexId=keep.id`) | merge | 무조건 |

**"parseNaver 자동채우기·승격"이 별도 지점으로 없는 이유**: 코드
추적 결과 "정보 자동 채우기"(`data-lstfill`)는 기존 주석에 이미
명시된 대로("listing 객체가 아니라 아직 DOM 입력값만 바꾼다")
DOM 입력 필드만 채울 뿐 `listing` 객체를 직접 건드리지 않는다 —
최종 저장은 항상 그 값을 그대로 들고 수정모드 "저장"(지점 4, edit
경로)을 눌러야 반영된다. 즉 자동채우기는 edit 경로에 흡수돼 있어
중복 배선이 불필요. `applyComplexPromotion()`(단지 "승격") 역시
`complexes[]`의 loc·parking·households만 다루고 `listings[].
deposit/listingStatus`는 전혀 건드리지 않아 무관함을 함수 본문
확인으로 검증했다. 스펙의 7개 시나리오 문구는 8개 코드 지점(6개
source 값)으로 정확히 커버된다고 판단 — 임의 축소가 아니라 실제
코드 흐름을 근거로 한 결론임을 명시해둔다.
...
function recordListingHistory(listing,source){ ...무조건 push... }
```
`create`/`merge`는 "변경"이 아니라 "최초 상태 기록"/"이관 사실
기록"이라는 별개의 의미라 무조건 append 헬퍼를 쓴다 — 특히 merge는
deposit·listingStatus 값 자체는 그대로여도 "다른 단지로 옮겨갔다"는
사실을 남겨야 하므로, `recordListingHistoryIfChanged`를 쓰면 값이
안 바뀌었다는 이유로 기록이 누락되는 버그가 생겼을 것이다.

### 표시 (2번째 커밋, `js/properties.js`만·신규 CSS 0)

B-117 매물 상세 사이드 패널에 "변동 이력" `h3` 섹션을 "전세 안전
체크" 다음에 추가. `listingHistoryHTML(l)`이 `history[]`를 시간
역순으로 뒤집어 "7/19 4.5억·게시중 (최초 등록)" 형식으로 렌더 —
날짜는 `M/D`, 가격은 `N억`(없으면 "가격 미정"), 상태·출처 라벨은
`esc()` 이스케이프. 기존 `.cx-listing-meta`/`.tnum` 클래스만
재사용해 신규 CSS 0줄. 시각화·추세 화살표는 스펙 3번 그대로 B-42
몫으로 남겨 이번엔 순수 리스트만 구현했다.

### 검증 (Playwright, 42개 체크 — 데스크톱 1440×900+모바일 390×844)

- **create**: `saveAsComplexListing()`을 직접 호출해 신규 단지+
  매물을 생성 → `history` 1건·`source:'create'`·초기 deposit/status
  일치 확인. 나머지 2개 create 지점(TSV 임포트·마이그레이션
  위저드)은 완전히 동일한 인라인 패턴(`history:[{at,deposit,
  listingStatus,source:'create'}]`)이라 UI 트리거 대신 코드 재검토로
  확인 — 각 트리거(TSV 붙여넣기 전체 플로우, 레거시 마이그레이션
  UI)를 별도로 재현하는 비용 대비 실익이 낮다고 판단한 의도적 선택.
- **edit**: 수정모드에서 deposit을 3.0→3.8로 바꾸고 저장 → append
  확인. **동일 값(3.8)으로 재저장 → 무append 확인**(중복 방지 핵심
  검증).
- **check**: 상태를 '사라짐'으로 만든 뒤 "게시중 확인" 클릭 →
  append(`source:'check'`). **이미 '게시중'인 상태에서 재클릭 →
  무append**.
- **gone**: "사라짐 처리" 클릭 → append(`source:'gone'`).
- **price**: `prompt()`에 5.1 입력 → append(`source:'price'`,
  deposit=5.1, status='가격변동'). **같은 값(5.1) 재입력 → 무append**.
- **merge**: cxB에 매물을 하나 만들고 cxA로 병합 실행 → 이관된
  매물에 `history` 1건·`source:'merge'` 확인(deposit/status는
  이관 전후 동일값인데도 기록됨 — 무조건 append 헬퍼가 의도대로
  동작).
- **`applyGuards` 무손실 3케이스**: ①`history` 필드 자체가 없는
  기존 매물 → 소급 생성 없이 빈 배열 ②`history`가 배열이 아닌
  값(문자열)으로 오염 → 빈 배열로 안전 보정 ③기존에 실제 `history`
  1건이 있는 매물 → 재적용해도 그대로 보존(무손실).
- **XSS**: `listingStatus`에 `<img src=x onerror=...>` payload를
  직접 주입(코드상 select로 제한돼 있어 정상 경로로는 불가능하지만
  방어 계층 확인 차원) → 상세 패널 재렌더 시 `esc()`로 이스케이프돼
  스크립트 미실행 확인.
- **회귀**: B-27-lite 안전 체크 섹션 정상 렌더 무회귀, B-118 병합
  실행 시 drop 단지(cxB) 삭제 등 기존 동작 그대로 무회귀.
- **Redis 왕복**: `sh_token` 세팅 후 `save()`+`flushPendingSync()`
  강제 발화, `/api/state` POST를 mock 서버로 가로채 바디에
  `"source":"merge"` 문자열이 포함됨을 실제 요청에서 확인(guest
  모드는 저장을 건너뛰므로 이 구간만 비guest로 전환해 테스트).
- `node --check` 13개 js 파일 전부 통과. 스크래치 테스트 스크립트는
  실행 후 삭제(레포 무잔존).

**→ B-43 완료·push 완료(`59ddcde`/`41de716`)**. 다음은 B-42(가격
타임라인 시각화)가 별도 지시 대기 — 이번에 쌓기 시작한
`listings[].history[]`가 그 데이터 소스가 된다.

---

## 2026-07-20 — B-42 가격 타임라인 — history[] 기반 추세 표시 (CRM 심화층 종료)

커맨드센터 확정 스펙으로 착수. B-43이 쌓기 시작한 `listings[].
history[]`를 **읽기만 하는 순수 표시 기능** — 저장·판정·정렬
개입 전혀 없음, 스키마 무변경. 손 B 부재로 파일 락 제약 없이
진행, 지시대로 1커밋(`33cbe2e`).

### 추세 배지 (`depositTrendBadge(l)`, `js/properties.js`)

매물 행(`renderCxListings`)과 단지 카드 헤드라인(`renderComplexes`
의 `.c-price`, 대표매물 기준) 두 곳에 삽입. 알고리즘: `history`를
`deposit!=null`로 걸러 **시간순으로 가장 최근 인접 2건**(배열의
마지막 두 원소)을 비교 — `Math.round((cur-prev)*100)/100`으로
반올림한 뒤 0이면 무표시. 반올림을 먼저 하는 이유는 부동소수점
연산(`4.5-4.3`이 `0.19999999999999973`이 되는 등)이 그대로 표시
로직을 타면 "↓0억" 같은 의미 없는 배지가 뜨거나, 반대로 실제로는
같은 값인데 반올림 전 상태로 비교해 미세한 배지가 뜨는 오류로
이어질 수 있어서다. 반올림 후 0 비교는 두 효과를 동시에 낸다:
①부동소수점 오차 흡수 ②상태만 바뀌고 가격은 그대로인 `edit`류
기록이 최근 인접 2건에 포함돼도 자연히 무표시(가격이 실제로
안 바뀌었으니 배지도 없어야 맞음). 색상은 하락=`.chip.ok`(긍정,
기존 초록 계열), 상승=`.chip.warn`(주의, 기존 주황 계열) — 둘 다
새 클래스 없이 재사용. `deposit` 있는 이력이 2건 미만이면 무표시
(소급·추정 금지 원칙 — B-43이 막 배선된 시점이라 대부분의 기존
매물은 아직 이력이 0~1건뿐이고, 그래서 지금은 배지가 안 보이는
게 정상 상태임을 스펙에서도 명시).

### 스파크라인 (`listingSparklineHTML(l)`, `js/properties.js`)

B-117 상세 패널의 "변동 이력" 리스트 바로 위에 삽입. 외부 차트
라이브러리를 쓰지 않고 인라인 `<svg>`를 문자열로 직접 조립 —
`viewBox="0 0 240 40"` 고정, `deposit` 값 배열을 min/max로
0~1 정규화한 뒤 `circle`(점)+`path`(꺾은선, `M`/`L` 명령) 좌표를
계산한다. 전 구간 값이 완전히 동일하면(`range=max-min`이 0)
`||1`로 안전하게 폴백해 가운데 일직선을 그리고 크래시하지 않는다.
좌표 문자열은 전부 `Math.round`/`toFixed`가 만든 숫자뿐이라 사용자
텍스트가 섞일 경로가 원천적으로 없다 — 기존 코드가 `${rep.deposit}`
같은 순수 숫자 보간에 `esc()`를 쓰지 않는 관례(`priceHTML` 등)와
동일하게 여기도 `esc()`를 생략했고, 이 판단이 맞는지 Playwright로
`path`의 `d` 속성을 정규식(`M[-\d.]+,[-\d.]+( L[-\d.]+,[-\d.]+)*`)
으로 검증해 재확인했다. CSS는 `.cx-listing-spark{width:100%;
height:40px}` 1줄만 추가 — 내부 좌표계는 `viewBox`로 고정하고
실제 렌더 폭만 카드 폭에 맞춰 반응형으로 늘어나게 한다.

### 검증 (Playwright, 40개 체크 — 데스크톱 1440×900+모바일 390×844)

단지 하나에 매물 5개를 이력 개수별로 구성한 fixture — 0건, 1건,
2건(하락: 4.5→4.3), 2건(상승: 3.0→3.1), 5건(혼합: 2.0→2.5→2.5
(상태만 바뀐 gone, 가격 불변)→2.3→2.3(상태만 바뀐 check, 가격
불변) — **일부러 마지막 두 값을 동일하게 만들어 "이력이 여러
건이어도 최근 값이 같으면 무표시"를 검증**하도록 설계).

- 0·1건: 배지 없음 확인.
- 2건 하락: "↓0.2억" 정확 표시, DOM 클래스에 `ok` 포함 확인.
- 2건 상승: "↑0.1억" 정확 표시, DOM 클래스에 `warn` 포함 확인.
- 5건 혼합(마지막 두 값 동일): 이력이 5건이나 있어도 배지
  없음 확인 — "이력 개수"가 아니라 "최근 인접 두 값이 다른지"가
  진짜 판정 기준임을 실측으로 증명.
- 단지 카드 헤드라인에도 대표매물(하락 매물로 지정) 기준 동일
  배지가 뜨는지 확인.
- 스파크라인: 2건 미만이면 SVG 자체가 렌더되지 않음(0개), 2건
  이상이면 정확히 1개 렌더. `circle` 개수가 이력 건수(5개 fixture
  기준 5개)와 정확히 일치. `path`의 `d` 속성이 숫자·`M`/`L`
  명령만으로 구성됨을 정규식으로 재확인(주입 경로 없음 재검증).
  전 구간 동일값(0,0)을 넣어도 `NaN`이 문자열에 섞이지 않고
  안전하게 렌더됨을 별도 직접 호출로 확인.
- **회귀**: B-43 "변동 이력" 리스트가 스파크라인과 같은 패널에
  공존해도 정상 렌더(무회귀), B-27-lite 전세 안전 체크 섹션도
  무회귀.
- **Redis 왕복(스키마 무변경 재확인)**: `sh_token` 세팅 후
  `save()`+`flushPendingSync()` 강제 발화, `/api/state` POST를
  mock으로 가로채 listing 객체의 키 목록을 검사 — `history`는
  그대로 있고 `trend`/`spark`/`sparkline` 등 새 필드가 전혀
  추가되지 않았음을 확인해 "표시 전용, 스키마 무변경" 스펙을
  실측으로 재확인했다.
- `node --check` 13개 js 파일 전부 통과. 스크래치 테스트 스크립트는
  실행 후 삭제(레포 무잔존).

**→ B-42 완료·push 완료(`33cbe2e`)**. CRM 심화층(B-41 임장 노트→
...
## 2026-08-07 — B-193R: 이미지 첨부 레이스 + 부수 2건 (커밋 2개)

①`compressImage`가 성공·실패 무관하게 콜백을 정확히 한 번 부르도록(실패는 `cb(null)`, 0×0 디코드도 실패 처리 — `toDataURL`이 "data:,"라는 깨진 값을 돌려주기 때문) + 소비처 3곳(`scApplyPreviewThumb`·`f_img`·첨부 경로) 가드. ②폼별 진행 건수(pending)+세대 토큰(gen)을 도입해 저장이 압축 완료를 기다렸다 자동으로 이어지게 하고("사진 처리 중…" 버튼 표시, 재클릭 무시), 세대가 바뀐 뒤 도착한 콜백은 폐기. 3진입로(파일 선택·붙여넣기·수정 모달) 전부 `scAttachImageFiles` 하나로 통합. 역검증: 첨부 직후 즉시 저장 시 카드 썸네일이 사용자 사진(600×450 초록)으로, 진단 때의 og:image(320×320 파랑) 점령이 사라짐(`fac9718`/`30e0b9a`).
...
## 2026-08-08 — B-183: 라이트박스 720px 상한 적용 (커밋 1개)

`.sc-lightbox-box{max-width:720px}`(클래스 1개)가 `.modal .box{max-width:520px}`(클래스 2개)에 특이도로 밀려 무시되고 있었다. `!important`나 ID로 올리지 않고 **선택자 형태를 이긴 쪽과 같게** 맞춰(`.modal .sc-lightbox-box`) 해결 — 뒤에 선언돼 있어 동점이면 이긴다. 실측: 890/1280/1440px 전부 박스 폭 520→**720px**, 모바일 390px는 350px 그대로(뷰포트가 작아 상한 무관). 좌우 넘김(1/3→2/3→3/3→순환), 1장일 때 화살표 숨김, 닫기 버튼·ESC 전부 회귀 0(`e6dd30d`).

---

## 2026-08-08 — B-184: 저장 띠·하단 앱바 중복 노출 (커밋 1개)

저장 띠를 숨기는 분기가 `max-width:760px`인데 하단 앱바는 `max-width:899.98px`이라, **761~899.98px에서 둘이 동시에 떴다**. 해당 규칙만 앱 표준 경계 블록으로 옮김(B-148 전례 — 그 블록의 나머지는 소형 화면 타이포·간격 전용이라 경계를 통째로 옮기면 광범위한 시각 변화가 생기므로 저장 띠 2줄만 이동). 중복 선언이던 `font-size`는 기본 규칙과 같은 값이라 제거. 노출 매트릭스 실측(390/760/761/830/899/900/1280) — 수정 전 761·830·899에서 동시 노출 true였고 수정 후 전 구간 **동시 노출 0**, 각 구간에서 의도된 쪽만 노출. B-172 치명적 안내(`.is-err`) 예외는 전 구간에서 그대로 노출됨을 별도 실증(`cf0b057`).

---

## 2026-08-08 — B-185: applyGuards 커버리지 + inboxId 보존 (커밋 1개)

①**배열 자체 가드**: `chatHistory`·`regNews`·`savedRoutes`·`prep`·`steps`가 `||[]`라 truthy 비배열(문자열·객체) 오염을 그대로 통과시켰다 → 전부 `guardArr`. `prep`/`steps`는 아래 이관 블록이 `.forEach`로 **먼저** 읽으므로 가드를 그 앞으로 배치(안 그러면 거기서 먼저 깨짐). ②**항목 가드** `guardItems()` 신설 — 배열은 맞는데 안에 `null`·문자열이 섞이면 이어지는 `.map`에서 속성 접근만으로 TypeError가 나 applyGuards 전체가 멈추고 화면이 빈 채로 선다. 정규화가 아니라 **폐기**(정상 항목 보존) + 경고 1줄. complexes·listings·scraps·actions·assets.items 5곳 적용. ③**inboxId 보존**: 스크랩 가드가 객체를 통째로 새로 만들어 거기 없는 필드는 로드마다 사라졌는데, `inboxId`(B-158)가 그랬다 — 유실되면 pullInbox 중복 방지가 id prefix 폴백에만 의존하게 된다. 문자열일 때만 보존 + JSDoc 등재. **지시서 밖 발견 1건**: B-30 `assignee`/`due` 보정이 prep 이관 블록보다 앞에 있어 이관으로 새로 생긴 액션만 그 필드 없이 저장됐다가 다음 로드에서야 채워졌다(같은 state를 두 번 통과시키면 결과가 달라짐 = 왕복 diff 0 위반) → 이관 뒤로 이동해 한 번에 수렴. 검증: 오염 백업(비배열 5필드·null/문자열/숫자 항목) 로드 → **크래시 0·정상 항목 전부 보존·폐기 경고 15건·화면 정상 렌더**, 정상 state 왕복 diff **0**(수정 전엔 actions 불일치), inboxId 왕복 보존 실증(직접 담은 스크랩은 undefined 유지), 같은 인박스 항목 재수신 시 중복 0·신규만 추가. 로그인·게스트 양 경로 5탭 회귀 0(`03f465d`).

---

## 2026-08-08 — B-199: 수집함 읽기 모달 (커밋 1개)

카드 클릭이 곧장 편집기를 열던 걸 "읽기"로 바꿨다 — GPT 리서치처럼 긴 글은 편집기 안에서 읽어야 해서 마찰이 컸다(B-150 재개 건). 읽기 전용 모달 신설(기존 `.modal`+`openModal/closeModal` 재사용): 제목·유형/상태 배지·메타 칩·이미지 썸네일(탭 시 기존 `scOpenLightbox` 연동)·**B-198 인스타 임베드 재사용**(`scSyncIgEmbed` — 읽기 모달이 인스타 소비의 주 지점이 됨)·`renderMd` 전문 렌더(본문 높이 제한 없이 모달 스크롤). 하단은 삭제·상태 select·닫기·수정(읽기 닫고 `openScEdit` 전환). 진입은 갤러리 카드 클릭(편집 직행 대체)과 리스트 카드 본문 클릭(지금까지 무동작이라 갤러리와 비대칭이던 영역 — B-190 검증 때 확인된 그 비대칭 해소) 양쪽. 카드 안의 기존 인터랙션(링크 칩·원문 토글·상태 select·개별 버튼·선택 모드 캡처)은 `closest()` 제외 목록으로 그대로 분리 보존. 핸들러는 모듈 로드 시 1회만 바인딩(렌더마다 누적 방지). state 무변경 — 전부 렌더 파생물. 검증: 마크다운 표(th 3·td 6)·목록·인용·강조 렌더, 수정 전환 후 저장 반영, 인스타 임베드 표시+닫을 때 iframe 0, 이미지→라이트박스 위에 겹치고 ESC가 한 겹씩(라이트박스→읽기), 링크 칩·상태 변경·원문 클릭·선택 모드·개별 삭제 전부 읽기 모달 안 열림, 모바일 390px 긴 문서(scrollHeight 7,643) 스크롤·닫기 정상, 가로 넘침 0, 콘솔 에러 0(`83ca679`).

---

## 2026-08-08 — B-200·B-201·B-202: 수집함 소형 3건 (커밋 1개)

**B-200 유형 기본값 기억**: 마지막으로 저장한 유형을 `sh_scLastType`(state 스키마 밖 UI 키, `sh_propViewMode` 전례)에 기록해 다음 폼 오픈의 기본 선택으로. 적용 지점을 `scClearForm`뿐 아니라 `scOpenForm`에도 둔 이유 — 새로고침 직후 첫 오픈은 clear를 거치지 않아 index.html의 첫 칩(청약공고)이 그대로 남는다(검증 중 발견). SC_TYPE에 없는 오염값·삭제 시 `subscription` 폴백, 유형별 필드 숨김도 함께 반영. **B-201 갤러리 썸네일 고정 높이**: `aspect-ratio:4/5`라 카드가 넓을수록 이미지가 같이 커져(1280px에서 740px) 사진 유무에 따라 카드 높이가 **628px** 벌어졌다 → `--sc-gallery-media-h:180px` 신설해 이미지·프리뷰 블록 공용 고정 높이+cover. 미디어 높이 편차 0, 카드 높이 편차 628→22px(잔여는 이미지가 아니라 텍스트 줄 수 차이). 라이트박스 무접촉. **B-202 붙여넣기 임포트 제거**: 실사용 0(사용자 확인) — 트리거 버튼·`scImportModal` 마크업·`.sc-import-*` CSS·파서 일체(frontmatter/표/블록/자유텍스트) 제거, 잔여 참조 grep 0. ⚠️ `js/scraps-import.js` **파일 자체는 삭제하지 않았다** — 이 파일에 임포트와 무관한 규제·뉴스 렌더(`renderRegNews`/`updateDashRegline`)가 함께 있고 `boot.js renderAll()`·`nav.js renderDash()`가 부르는 살아있는 코드라, 지우면 즉시 깨진다(파일 상단에 사유 주석). 함정 1건: 폼 닫기 ✕ 버튼이 `.sc-import-trigger` 클래스를 공유하고 있어 클래스 삭제 시 font-weight·flex-shrink가 빠진다 — 인라인으로 동일 값 보존. 전체 백업 가져오기(`#importBtn`)와 단지 병합 표(`.import-tbl`)는 이름만 비슷한 별개 기능이라 무접촉 확인. 회귀: 추가 저장·수정 모달·인박스·읽기 모달·규제·뉴스 렌더 전부 정상, 콘솔 에러 0(`3dc2d06`).
